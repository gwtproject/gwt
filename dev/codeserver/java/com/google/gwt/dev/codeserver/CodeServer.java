...
public class CodeServer {
  ...
  public static void main(Options options) {
    if (options.isCompileTest()) {
      ...
      try {
        ...
        outboxes = makeOutboxes(options, logger, minimalRebuildCacheManager, unitCache);
      } catch (Throwable t) {
        t.printStackTrace();
  ...
  public static WebServer start(Options options) throws IOException, UnableToCompleteException {
    ...
    MinimalRebuildCacheManager minimalRebuildCacheManager =
        new MinimalRebuildCacheManager(baseCacheDir);
    OutboxTable outboxes =
        makeOutboxes(options, startupLogger, minimalRebuildCacheManager, unitCache);

    JobEventTable eventTable = new JobEventTable();
    JobRunner runner = new JobRunner(eventTable, outboxes, minimalRebuildCacheManager);
  ...
  private static OutboxTable makeOutboxes(Options options, TreeLogger logger,
      MinimalRebuildCacheManager minimalRebuildCacheManager, UnitCache unitCache)
    ...
    for (String moduleName : options.getModuleNames()) {
      ...
      Recompiler recompiler = new Recompiler(outboxDir, launcherDir, minimalRebuildCacheManager,
          moduleName, options, unitCache);
