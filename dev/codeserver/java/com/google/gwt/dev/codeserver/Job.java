...
class Job {
  ...
  private JobEvent makeEvent(Status status, String message) {
    ...
    out.setTags(tags);
    out.setPerformanceCounts(getPerformanceCounts());
    return out.build();
  }

  private ImmutableMap<String, Long> getPerformanceCounts() {
    TreeLogger logger = getLogger();
    if (logger instanceof AbstractTreeLogger) {
      return ((AbstractTreeLogger)logger).getPerformanceCounts();
    }
    ...
    synchronized TreeLogger get() {
      if (child == null) {
        ...
        if (child instanceof AbstractTreeLogger) {
          ((AbstractTreeLogger)child).clearPerformanceCounters();
        }
      }
      return child;
    }
