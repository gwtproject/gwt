...
public class JobRunner {
   ...
   */
  void clean(final TreeLogger logger, final OutboxTable outboxes) throws ExecutionException {
    try {
      TreeLogger branch = logger.branch(TreeLogger.INFO, "Cleaning disk caches.");
      executor.submit(new CleanerJob(branch, outboxes)).get();
  ...
  private class CleanerJob implements Callable<Void> {
    ...
    public CleanerJob(final TreeLogger logger, final OutboxTable outboxes) {
      this.logger = logger;
    ...
    public Void call() throws UnableToCompleteException {
      ...
      outboxes.forceNextRecompileAll();
