...
(function() {
  ...
  function showModuleDialog(codeserver_url) {
    ...
    function makeModuleRows(mod) {
      ...
      cell.appendChild($doc.createTextNode(module_name));
      ...
      if (error) {
        ...
        var text = mod.superdevmode ? "On (" : "Off (";
        cell.appendChild($doc.createTextNode(text));

        var text = mod.superdevmode ? "turn off" : "turn on";
        var toggle = $doc.createElement('a');
        toggle.appendChild($doc.createTextNode(text));
