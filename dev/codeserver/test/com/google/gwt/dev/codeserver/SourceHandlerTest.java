...
public class SourceHandlerTest {
  ...
  public void testSourceMapPathInfo() {
    ...
    String strongName = Util.computeStrongName(String.valueOf(new Random().nextLong()).getBytes());
    ...
    assertTrue(srcHand.isSourceMapRequest(SOURCEMAP_PATH + moduleName + "/" + strongName + SOURCEMAP_SUFFIX));
    ...
    assertEquals(moduleName, srcHand.getModuleNameFromRequest(SOURCEMAP_PATH + moduleName + "/" + strongName + SOURCEMAP_SUFFIX));

    assertEquals(strongName, srcHand.getStrongNameFromRequest(SOURCEMAP_PATH + moduleName + "/" + strongName + SOURCEMAP_SUFFIX));
    assertNull(srcHand.getStrongNameFromRequest(SOURCEMAP_PATH + moduleName + "/invalid_hash" + SOURCEMAP_SUFFIX));
  }
}
