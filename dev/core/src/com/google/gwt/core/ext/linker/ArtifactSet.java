...
public final class ArtifactSet implements SortedSet<Artifact<?>>, Serializable {
  ...
  private SortedSet<Artifact<?>> treeSet =
      new TypeIndexedSet<Artifact<?>>(new TreeSet<Artifact<?>>());
  private final TypeIndexedSet.TypeIndex typeIndex = ((TypeIndexedSet) treeSet).getTypeIndex();
  ...
  public <T extends Artifact<? super T>> SortedSet<T> find(Class<T> artifactType) {
    return typeIndex.findAssignableTo(artifactType);
  }
   ...
   */
  public void freeze() {
    if (treeSet instanceof TypeIndexedSet<?>) {
