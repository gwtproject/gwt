...
/**
 * Serialize/Deserialize EntityDescriptor instances to json and as artifacts
 *
 * @author ocallau@google.com (Oscar Callau)
 */
public class EntityDescriptorJsonTranslator {

  public final static String ARTIFACT_NAME = "entities";

  private static JSONObject basicJsonEntity(EntityDescriptor entity) throws JSONException {
    JSONObject json = new JSONObject();
    JSONArray frags = new JSONArray();
    for (EntityDescriptor.Fragment frg : entity.getFragments()) {
      JSONObject frag = new JSONObject();
      frag.put("id", frg.id);
      frag.put("size", frg.size);
  ...
  private static JSONObject basicJsonMember(MemberDescriptor entity) throws JSONException{
    JSONObject json = basicJsonEntity(entity);
  ...
  public static JSONObject writeJson(PackageDescriptor pkg) throws JSONException {
    ...
    for (ClassDescriptor cd : pkg.getClasses()) {
      JSONObject cls = basicJsonEntity(cd);
      cls.put("name", cd.getName());
      //fields
      JSONArray fs = new JSONArray();
      for (FieldDescriptor fd : cd.getFields()) {
        JSONObject fld = basicJsonMember(fd);
        fs.put(fld);
      }
      cls.put("fields", fs);
      //methods
      ...
      for (MethodDescriptor md : cd.getMethods()) {
        JSONObject mth = basicJsonMember(md);
  ...
  private static class Deserializer {
    ...
    private void updateEntity(EntityDescriptor entity, JSONObject jsonObject) throws JSONException{
      ...
      for (int i = 0; i < frags.length(); i++) {
        ...
        Fragment fragment = entity.new Fragment();
        fragment.id = frag.getInt("id");
        fragment.size = frag.getInt("size");
