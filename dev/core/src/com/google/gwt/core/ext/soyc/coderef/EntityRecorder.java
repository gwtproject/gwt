...
public class EntityRecorder {
  ...
  public static final String FRAGMENT_POINT = "splitPoint";
  ...
  private JSONObject getSizeMetrics(int fragment) throws JSONException {
    ...
    obj.put(FRAGMENT_SIZE, this.fragSizes[fragment][FRAG_SIZE_IDX]);
    obj.put(FRAGMENT_STR_VAR, this.fragSizes[fragment][OTHER_SIZE_IDX]);
    return obj;
  }
  ...
  private void recordCodeReferences(DependencyGraphRecorder codeGraph,
    ...
    addArtifactFromJson(
        EntityDescriptorJsonTranslator.writeJson(
            PackageDescriptor.from(codeGraph.getCodeGraph())),
  ...
  private void recordFragments(JProgram jprogram) throws IOException, JSONException {
    ...
    // split points
    JSONArray fragments = new JSONArray(this.sizeMetrics);
    for (JRunAsync runAsync : jprogram.getRunAsyncs()) {
      ...
      JSONObject splitPoint;
      if (fragId > 0 && fragId < sizeMetrics.length) {
        splitPoint = this.sizeMetrics[fragId];
      } else {
        // If each splitPoint's id correspond to a valid fragment, we can safely remove this case
        splitPoint = new JSONObject();
        splitPoint.put(FRAGMENT_ID, fragId);
        fragments.put(splitPoint);
      }
      splitPoint.put(FRAGMENT_POINT, runAsync.getName());
    }
    jsonPoints.put(FRAGMENTS, fragments);
