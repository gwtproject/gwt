...
public class MethodDescriptor extends MemberDescriptor {
  ...
  private Set<MethodDescriptor> dependantMethods = Sets.newIdentityHashSet();
  private int uniqueId;
  private String paramTypes;
  private Set<JMethod> methodReferences = Sets.newIdentityHashSet();
  ...
  public void addDependant(MethodDescriptor methodDescriptor) {
    dependantMethods.add(methodDescriptor);
  }
  ...
  /**
   * Returns the list dependant ids.
   */
  public int[] getDependantPointers() {
    int[] ps = new int[dependantMethods.size()];
    int c = 0;
    for (MethodDescriptor dependant : dependantMethods) {
      ps[c++] = dependant.getUniqueId();
    }
    return ps;
  }

  public Set<MethodDescriptor> getDependantMethods() {
    return Collections.unmodifiableSet(dependantMethods);
  }
