...
public class PackageDescriptor {
  ...
  public final static String DEFAULT_PKG = "<default>";
  ...
  protected ArrayList<PackageDescriptor> pkgs = Lists.newArrayList();
  protected ArrayList<ClassDescriptor> cls = Lists.newArrayList();
  ...
  private void condense() {
    if (pkgs.size() == 1 && cls.size() == 0) {
      PackageDescriptor child = pkgs.get(0);
    ...
    for (PackageDescriptor pkg : pkgs) {
      pkg.condense();
    }
  }
  ...
  protected void computeSubPackages(Collection<ClassDescriptor> values) {
    ...
    for (Entry<String, ArrayList<ClassDescriptor>> entry : subPkgs.entrySet()) {
      pkgs.add(newFrom(entry.getValue(), entry.getKey(),
