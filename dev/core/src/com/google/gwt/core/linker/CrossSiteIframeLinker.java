...
public class CrossSiteIframeLinker extends SelectionScriptLinker {
   ...
   * ending with ".js".)
   *
   * <p>The default implementation disallows Super Dev Mode on https pages. It could be overridden
   * to implement a blacklist or whitelist of hostnames where Super Dev Mode may run. As a safety
   * precaution, it's recommended to return false for https and for the hostnames in URL's visited
   * by end users.
   *
   * <p>If you override this method to allow https, it probably won't work because
   * browsers often disallow loading JavaScript from http URL's into https pages. To make it work,
   * you would also have to find a way to run the code server using https.
   */
