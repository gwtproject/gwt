...
public class Compiler {
  ...
  public boolean run(TreeLogger logger) throws UnableToCompleteException {
    ...
    for (String moduleName : options.getModuleNames()) {
      ...
      modules[i++] = module;
      if (!maybeRestrictProperties(logger, module, options.getRestrictedProperties())) {
        return false;
      }
    }
  ...
  public static boolean maybeRestrictProperties(TreeLogger logger, ModuleDef module,
    ...
    try {
      for (Entry<String, Collection<String>> property : restrictedProperties.asMap().entrySet()) {
        ...
        if (bindingProp != null) {
          ...
          if (configProp.allowsMultipleValues()) {
            ...
            if (propertyValues.size() > 1) {
              logger.log(TreeLogger.WARN, "property '" + propertyName
                  + "' is a single-valued configuration property. " + "Set to the first value '"
                  + firstValue + "'.");
            }
