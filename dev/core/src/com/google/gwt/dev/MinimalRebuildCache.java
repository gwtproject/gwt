...
public class MinimalRebuildCache implements Serializable {
   ...
   * recreation of it's cached JS without also rerunning the Generator that creates type Foo.
   */
  private void appendTypesToRegenerateStaleGeneratedTypes(Set<String> staleTypeNames) {
    ...
    Set<String> staleGeneratedTypeNames =
        Sets.intersection(staleTypeNames, reboundTypeNamesByGeneratedTypeName.keySet());
    do {
      // Accumulate generatedTypes -> generators -> generatorTriggeringTypes.
      Set<String> typesThatRebindTypesThatGenerateStaleTypes = computeTypesThatRebindTypes(
          computeReboundTypesThatGenerateTypes(staleGeneratedTypeNames));
      // Mark these generator triggering types stale.
      staleTypeNames.addAll(typesThatRebindTypesThatGenerateStaleTypes);

      // It's possible that a generator triggering type was itself also created by a Generator.
      // Repeat the backwards trace process till none of the newly stale types are generated.
      staleGeneratedTypeNames = Sets.intersection(typesThatRebindTypesThatGenerateStaleTypes,
          reboundTypeNamesByGeneratedTypeName.keySet());
