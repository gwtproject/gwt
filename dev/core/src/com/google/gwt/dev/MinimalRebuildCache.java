...
public class MinimalRebuildCache implements Serializable {
  ...
  private final Multimap<String, String> containedTypeNamesByUnitTypeName = HashMultimap.create();
  ...
  private final Multimap<String, String> unitTypeNameByContainedTypeName = HashMultimap.create();
   ...
   * <p>
   * Deleted types are all those types that are contained within compilation units that are known to
   * have been deleted.
   */
  public Set<String> computeDeletedTypeNames() {
    ...
    for (String deletedCompilationUnitName : deletedCompilationUnitNames) {
      deletedTypeNames.addAll(containedTypeNamesByUnitTypeName.get(deletedCompilationUnitName));
    }
   ...
   * <p>
   * Modified types are all those types that are contained within compilation units that are known
  ...
  public Set<String> computeModifiedTypeNames() {
    ...
    for (String modifiedCompilationUnitName : modifiedCompilationUnitNames) {
      modifiedTypeNames.addAll(containedTypeNamesByUnitTypeName.get(modifiedCompilationUnitName));
    }
  ...
  public void recordNestedTypeNamesPerType(CompilationUnit unit) {
    // For the root type in the compilation unit the source name and binary name are the same.
    String unitTypeName = unit.getTypeName();

    containedTypeNamesByUnitTypeName.removeAll(unitTypeName);
    for (CompiledClass compiledClass : unit.getCompiledClasses()) {
      String containedTypeName = InternalName.toBinaryName(compiledClass.getInternalName());
      containedTypeNamesByUnitTypeName.put(unitTypeName, containedTypeName);
      unitTypeNameByContainedTypeName.put(containedTypeName, unitTypeName);
    }
  }
