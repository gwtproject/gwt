...
public class MinimalRebuildCache implements Serializable {
  ...
  private final Multimap<String, String> generatedCompilationUnitNamesByReboundTypeNames =
      HashMultimap.create();
  private final Predicate<String> hasJsPredicate;
  ...
  {
    hasJsPredicate = new Predicate<String>() {
      @Override
      public boolean apply(String typeName) {
        // You can't just check for the presence of JS for this type since stale types are
        // calculated and JS can be cleared multiple times during UnifyAst as Generators run.
        return lastReachableTypeNames.contains(typeName);
      }
    };
  }
  ...
  public Set<String> filterUnreachableTypeNames(Set<String> typeNames) {
    return Sets.newHashSet(Sets.filter(typeNames, hasJsPredicate));
  }
