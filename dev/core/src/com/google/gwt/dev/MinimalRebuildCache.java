...
public class MinimalRebuildCache implements Serializable {
  ...
  private final Multimap<String, String> inputResourcesByReboundTypeName = HashMultimap.create();
  ...
  private final Multimap<String, String> reboundTypeNamesByRebinderTypeName = HashMultimap.create();
  ...
  public void associateReboundTypeWithInputResource(String reboundTypeName,
    ...
    inputResourcesByReboundTypeName.put(reboundTypeName, inputResourcePath);
  }
  ...
  public void clearRebinderTypeAssociations(String rebinderTypeName) {
    Collection<String> reboundTypeNames = reboundTypeNamesByRebinderTypeName.get(rebinderTypeName);
    reboundTypeNamesByRebinderTypeName.removeAll(rebinderTypeName);
    for (String reboundTypeName : reboundTypeNames) {
      rebinderTypeNamesByReboundTypeName.remove(reboundTypeName, rebinderTypeName);
    }
  }

  public void clearReboundTypeAssociations(String reboundTypeName) {
    Collection<String> inputResources = inputResourcesByReboundTypeName.get(reboundTypeName);
    inputResourcesByReboundTypeName.removeAll(reboundTypeName);
    for (String inputResource : inputResources) {
      reboundTypeNamesByInputResource.remove(inputResource, reboundTypeName);
    }
  }
  ...
  public boolean knowsAboutResource(String fileName) {
    return reboundTypeNamesByInputResource.containsKey(fileName);
  }
  ...
  public void recordBuildResources(ModuleDef module) {
    ...
    for (Resource currentResource : currentResources) {
      ...
      if (!lastModifiedByResourcePath.containsKey(currentResource.getPath())) {
        // Added resource.
        modifiedResourcePaths.add(currentResource.getPath());
        lastModifiedByResourcePath.put(currentResource.getPath(),
            currentResource.getLastModified());
      } else if (lastModifiedByResourcePath.get(currentResource.getPath())
          != currentResource.getLastModified()) {
        // Modified resource.
        modifiedResourcePaths.add(currentResource.getPath());
        lastModifiedByResourcePath.put(currentResource.getPath(),
  ...
  public void recordRebinderTypeForReboundType(String reboundTypeName, String rebinderType) {
    ...
    reboundTypeNamesByRebinderTypeName.put(rebinderType, reboundTypeName);
  }
  ...
  private void appendTypesThatRebindTypes(Set<String> staleTypeNames,
    ...
    Set<String> rebinderTypeNames = Sets.newHashSet();
    for (String reboundTypeName : reboundTypeNames) {
      rebinderTypeNames.addAll(rebinderTypeNamesByReboundTypeName.get(reboundTypeName));
    }
    staleTypeNames.addAll(rebinderTypeNames);
  }
