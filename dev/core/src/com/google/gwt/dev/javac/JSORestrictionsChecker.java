...
public class JSORestrictionsChecker {
  ...
  public static final String ERR_MUST_EXTEND_MAGIC_PROTOTYPE_CLASS =
      "Classes implementing @JsInterface with a prototype must extend that interface's Prototype static inner class";
  ...
  private class JSORestrictionsVisitor extends SafeASTVisitor implements
    ...
    private void checkJsExport(MethodBinding mb, boolean allowed) {
      AnnotationBinding jsExport = GwtAstBuilder.getAnnotation(mb, GwtAstBuilder.JSEXPORT_CLASS);
    ...
    private void checkJsProperty(MethodBinding mb, boolean allowed) {
      AnnotationBinding jsProperty = GwtAstBuilder.getAnnotation(mb, GwtAstBuilder.JSPROPERTY_CLASS);
      if (jsProperty != null) {
        if (!allowed) {
    ...
    private void checkJsInterfaceMethodsForOverloads(Set<String> methodNames, ReferenceBinding binding) {
      for (MethodBinding mb : binding.methods()) {
        ...
        if (GwtAstBuilder.getAnnotation(mb, GwtAstBuilder.JSPROPERTY_CLASS) != null) {
          if (isGetter(methodName, mb) || isSetter(methodName, mb) || isHas(methodName, mb)) {
    ...
    private ClassState checkType(TypeDeclaration type) {
      ...
      if (String.valueOf(type.name).contains("Prototype")) {
        boolean xx = true;
      }
    ...
    private boolean checkClassImplementingJsInterface(TypeDeclaration type) {
      ...
      String jsPrototype = GwtAstBuilder.getAnnotationParameterString(GwtAstBuilder.getAnnotation(jsInterface,
          GwtAstBuilder.JSINTERFACE_CLASS), "prototype");
      if (!Strings.isNullOrEmpty(jsPrototype)) {
        checkClassExtendsMagicPrototype(type, jsInterface, true);
      } else {
        checkClassExtendsMagicPrototype(type, jsInterface, false);
      }
    ...
    private boolean isMagicPrototype(TypeDeclaration type, ReferenceBinding jsInterface) {
      return String.valueOf(type.name).equals("Prototype") && type.binding.enclosingType() == jsInterface;
    }

    /**
     * Walks up chain of interfaces and superinterfaces to find the first one marked with @JsInterface.
     */
    private ReferenceBinding findNearestJsInterface(ReferenceBinding binding) {
      if (isJsInterface(binding)) {
        return binding;
      }

      for (ReferenceBinding intb : binding.superInterfaces()) {
        ReferenceBinding checkSuperInt = findNearestJsInterface(intb);
    ...
    private ReferenceBinding findNearestJsInterfaceRecursive(ReferenceBinding binding) {
      ReferenceBinding nearest = findNearestJsInterface(binding);
      if (nearest != null) {
        return nearest;
  ...
  private boolean isJsInterface(TypeBinding typeBinding) {
    ...
    AnnotationBinding jsInterface = GwtAstBuilder.getAnnotation(typeBinding, GwtAstBuilder.JSINTERFACE_CLASS);
