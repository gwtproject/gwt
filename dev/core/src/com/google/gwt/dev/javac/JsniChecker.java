...
public class JsniChecker {
  ...
  private class JsniRefChecker extends JsVisitor {
    ...
    private void resolveClassReference(JsniRef jsniRef) {
      ...
      if (jsniRef.className().contains(".")) {
        // It is a qualified name, do not attempt to resolve.
        return;
      }
      // 1. Check to see if this name refers to the enclosing class.
      String declaringClass =
          JdtUtil.sourceNameFromNamesArray(method.binding.declaringClass.compoundName);
      if (declaringClass.endsWith(className)) {
        // Referring to declaring class name using unqualified name.
        jsniRef.setClassName(declaringClass);
        return;
      }
      // 2. Check to see if this name is one of the named imports.
      for (ImportReference importReference : cudImports) {
        String importedClass = JdtUtil.sourceNameFromNamesArray(importReference.getImportName());
        if (!importReference.isStatic()  && importReference.trailingStarPosition == 0 &&
            importedClass.endsWith(className)) {
          jsniRef.setClassName(importedClass);
          return;
        }
      }
      ...
      currentPackageClassName += (currentPackageClassName.isEmpty() ? "" : ".") +  className;
