  ...
  public static String getRelevantOptionsHash(JJSOptions options) {
    return StringUtils.toHexString(
        Md5Utils.getMd5Digest(
            Joiner.on('-').join(
                options.getJsInteropExportFilter().toString(),
                String.valueOf(options.shouldGenerateJsInteropExports())
        ).getBytes()));
  }
    ...
    if (instance == null) {
      ...
      if (usePersistent) {
        ...
        if (actualCacheDir != null) {
          try {
            return instance = new PersistentUnitCache(logger, actualCacheDir, getRelevantOptionsHash(options));
