...
public abstract class JavaToJavaScriptCompiler {
    ...
    private void optimizeJava(boolean singlePermutation) throws InterruptedException {
      if (options.getOptimizationLevel() > OptionOptimize.OPTIMIZE_LEVEL_DRAFT
        ...
        if (options.isOptimizePrecompile()) {
           ...
           * AST.
           */
          optimizeJavaOneTime("Early Optimization", 1, jprogram.getNodeCount(), new OptimizerContext());
        }
      }
    }
  ...
  protected final void optimizeJavaToFixedPoint() throws InterruptedException {
    ...
    OptimizerContext optimizerCtx = new OptimizerContext();
    optimizerCtx.buildCallGraph(jprogram);
    while (true) {
      passCount++;
      ...
      AstDumper.maybeDumpAST(jprogram);
      OptimizerStats stats = optimizeJavaOneTime("Pass " + passCount, passCount, nodeCount, optimizerCtx);
      allOptimizerStats.add(stats);
  ...
  private OptimizerStats optimizeJavaOneTime(String passName, int passCount, int numNodes,
    ...
    stats.add(MethodInliner.exec(jprogram, optimizerCtx, passCount).recordVisits(numNodes));
