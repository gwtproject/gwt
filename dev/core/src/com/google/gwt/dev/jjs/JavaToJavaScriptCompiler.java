  ...
  public static PermutationResult compilePermutation(TreeLogger logger,
    ...
    try {
      ...
      if (options.isRunAsyncEnabled()) {
        ...
        dependencyRecorder = chooseDependencyRecorder(options.isSoycEnabled() || isSourceMapsEnabled,
            jprogram, baos);
        ...
        if (baos.size() == 0 && options.isSoycEnabled()) {
          dependencyRecorder = recordNonSplitDependencies(jprogram, baos);
        }
        if (baos.size() > 0) {
          dependencies =
        ...
        if (dependencyRecorder == null || isSourceMapsEnabled) {
      ...
      if (isSourceMapsEnabled) {
        if (options.isClosureCompilerEnabled()) {
          ...
          toReturn.addArtifacts(SourceMapRecorder.makeSourceMapArtifacts(permutationId,
              sourceInfoMaps, jjsmap, sizeBreakdowns, ((DependencyGraphRecorder) dependencyRecorder),
              jprogram));
        }
      }
