...
/**
 * Compiles the Java <code>JProgram</code> representation into its corresponding library JavaScript
 * source.<br />
 *
...
public class LibraryJavaToJavaScriptCompiler extends JavaToJavaScriptCompiler {
  ...
  private class LibraryPermutationCompiler extends PermutationCompiler {
    ...
    @Override
    protected void removeEmptySuperCalls() {
      // Collapsing the super call chain can not be done accurately when constructing libraries
      // since method bodies of parent classes might or might not be loaded.
    }
  ...
  private class LibraryPrecompiler extends Precompiler {
    ...
    private void buildFallbackRuntimeRebindRules(
      ...
      for (JDeclaredType reboundType : reboundTypes) {
        ...
        fallbackRule.generateRuntimeRebindClasses(logger, module, context);
      }
    }
    ...
    private void buildLocalRuntimeRebindRules(StandardGeneratorContext context)
      ...
      Iterator<Rule> iterator = module.getRules().iterator();
      while (iterator.hasNext()) {
        Rule rule = iterator.next();
        if (rule instanceof RuleGenerateWith) {
          continue;
        }
        rule.generateRuntimeRebindClasses(logger, module, context);
      }
    }
    ...
    private void buildPropertyProviderRegistrator(
        ...
        throws UnableToCompleteException {
      SortedSet<BindingProperty> bindingProperties = module.getProperties().getBindingProperties();
      ...
      String propertyProviderRegistratorTypeName =
          propertyProviderRegistratorGenerator.generate(logger, context, module.getName());
    ...
    private void buildRuntimeRebindRegistrator(
      ...
      String runtimeRebindRegistratorTypeName =
          runtimeRebindRegistratorGenerator.generate(logger, context, module.getName());
      // Ensures that unification traverses and keeps the class.
    ...
    private Set<JDeclaredType> gatherReboundTypes(Collection<CompilationUnit> compilationUnits) {
    ...
    private Set<String> getTypeNames(Set<JDeclaredType> types) {
    ...
    private boolean runGenerator(RuleGenerateWith generatorRule, Set<String> reboundTypeNames,
      ...
      for (String reboundTypeName : reboundTypeNames) {
        ...
        generatorRule.generate(logger, module, context, reboundTypeName);
