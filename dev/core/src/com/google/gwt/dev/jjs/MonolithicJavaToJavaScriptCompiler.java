...
public class MonolithicJavaToJavaScriptCompiler extends JavaToJavaScriptCompiler {
  ...
  private class MonolithicPermutationCompiler extends PermutationCompiler {
    ...
    protected TypeMapper<?> normalizeSemantics() {
      ...
      try {
        ...
        TypeMapper<?> typeMapper = getTypeMapper(permutation);
        ResolveRuntimeTypeReferences.exec(jprogram, typeMapper, getTypeOrder(permutation));
        return typeMapper;
      } finally {
        event.end();
      }
  ...
  private TypeMapper getTypeMapper(Permutation permutation) {
  ...
  private TypeOrder getTypeOrder(Permutation permutation) {
    ConfigProps configProps = permutation.getProps().getConfigProps();
