...
public abstract class JDeclaredType extends JReferenceType {
   ...
   */
  public JMethod findMethod(String methodSignature) {
  ...
  /**
   * Returns the method with the given signature (if there is one) in this type or the nearest super
   * type.
   */
  public JMethod findMethodRecursive(String methodSignature) {
    for (JMethod method : getMethods()) {
      if (method.getSignature().equals(methodSignature)) {
        return method;
      }
    ...
    if (getSuperClass() != null) {
      return getSuperClass().findMethodRecursive(methodSignature);
    }
    return null;
  }
