...
public class JMethod extends JNode implements JMember, CanBeAbstract {
  ...
  public String getJsName() {
    JsMemberType actualJsMemberType = jsMemberType;
    for (JMethod method : getOverriddenMethodsIncludingSelf()) {
      if (method.jsName != null) {
        return method.jsName;
      }
      if (actualJsMemberType == JsMemberType.NONE && method.jsMemberType != JsMemberType.NONE) {
        actualJsMemberType = method.jsMemberType;
      }
    }
    return actualJsMemberType.computeName(this);
  }
