...
public class JTypeOracle implements Serializable {
  ...
  public static boolean isCastableByPrototype(JType type) {
  ...
  /**
   * True if the type can be cast across different Java types that are unrelated.
   */
  ...
  public boolean castFailsTrivially(JReferenceType fromType, JReferenceType toType) {
     ...
     */
    if (canCrossCastLikeJso(fromType) && canCrossCastLikeJso(toType) || isNoOpCast(toType)) {
      return false;
    }
