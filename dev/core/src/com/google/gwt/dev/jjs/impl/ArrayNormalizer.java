...
public class ArrayNormalizer {
  ...
  private class ArrayVisitor extends JModVisitor {
    ...
    /**
     * Returns the type for the last initialized dimension, e.g.
     * for int[4][3] => int
     * for int[4][5][] => int[]
     */
    private JType getTypeForLastInitializedDimension(JArrayType arrayType, int dims) {
      JType elementType = arrayType;
      do {
        elementType = ((JArrayType) elementType).getElementType();
