...
public class CallGraph {
   ...
   */
  class BuildCallGraphVisitor extends JVisitor {
    ...
    public void endVisit(JMethodCall x, Context ctx) {
      ...
      calleeCallersPairs.put(method, currentMethod);
      callerCalleesPairs.put(currentMethod, method);
    }
  ...
  /**
   * For removing a method, remove call the {caller->callees} and {callee->callers} pairs that are
   * related to the method.
   */
  public void removeMethod(JMethod method) {
    for (JMethod m : callerCalleesPairs.get(method)) {
      calleeCallersPairs.remove(m, method);
    }
  ...
  public Set<JMethod> getCallers(Set<JMethod> callees) {
    ...
    for (JMethod m : callees) {
      result.addAll(calleeCallersPairs.get(m));
    }
