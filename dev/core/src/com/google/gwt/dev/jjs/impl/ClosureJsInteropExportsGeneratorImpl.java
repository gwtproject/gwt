...
public class ClosureJsInteropExportsGeneratorImpl extends JsInteropExportsGenerator {
  ...
  private String ensureProvideCall(JDeclaredType x, String namespace, TreeLogger logger) {
  ...
  /**
   * Ensures that enclosing namespaces are exported with an empty ctor even if the enclosing type
   * doesn't have any exported members, since the <code>ctor.prototype</code> fields will need to
   * exist for @JsDoc declarations that refer to the virtual methods of the class.
   */
  @Override
  public boolean maybeExportTypeWithNoExportedMembers(JDeclaredType x, TreeLogger branch) {
    if (x.isJsType() && !x.hasAnyExports()) {
  ...
  private boolean tryChangeNamespace(String namespace) {
