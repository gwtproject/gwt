 ...
 */
public class ComputateOverridesAndImplementDefaultMethods {
  ...
  private void computeOverrides(JDeclaredType type, List<JMethod> newlyCreatedMethods) {
    ...
    for (String signature : interfaceMethodsBySignature.keySet()) {
      ...
      if (implementingMethod == null ||
          implementingMethod.getEnclosingType() != type ) {
        ...
        if (implementingMethod == null) {
          ...
          polymorphicMethodsByExtendedSignature.put(
              signature, interfaceMethods.iterator().next());
          continue;
        }
