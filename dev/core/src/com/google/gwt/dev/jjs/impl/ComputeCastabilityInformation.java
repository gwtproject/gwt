...
public class ComputeCastabilityInformation {
  private class AssignTypeCastabilityVisitor extends JVisitor {
    ...
    // Whether a cast/instanceof can succeed between fromType and toType.
    private boolean castSucceedsAtRuntime(JReferenceType fromType, JReferenceType toType) {
    ...
    private void computeCastMap(JReferenceType type) {
      ...
      for (JReferenceType castTargetType : castSourceTypesPerCastTargetType.keySet()) {
        if (!castSucceedsAtRuntime(type, castTargetType)) {
        ...
        for (JReferenceType castSourceType : castSourceTypes) {
          if (castSucceedsAtRuntime(type, castSourceType)
              && !JTypeOracle.isNoOpCast(castTargetType)) {
