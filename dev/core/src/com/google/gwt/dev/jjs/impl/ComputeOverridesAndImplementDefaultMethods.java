...
public class ComputeOverridesAndImplementDefaultMethods {
  ...
  private Multimap<String, JMethod> collectMostSpecificSuperInterfaceMethodsBySignature(
    ...
    for (String signature : signatures) {
      ...
      assert defaultMethods.size() <= 1 : "There should be at most one (leaf) default method but " +
          "there are " + defaultMethods.size() + "(" + defaultMethods + ")";
  ...
  private static Iterable<JDeclaredType> getAllImmediateSupers(JDeclaredType type) {
    if (type == null) {
      return Collections.<JDeclaredType>emptyList();
    }
