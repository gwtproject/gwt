  ...
  public class DeadCodeVisitor extends JModVisitor {
    ...
    public void endVisit(JFieldRef x, Context ctx) {
      ...
      if (x.hasClinit()) {
        multi.getExpressions()
            .add(createClinitCall(x.getSourceInfo(), x.getField().getEnclosingType()));
      }

      if (literal != null) {
        multi.getExpressions().add(literal);
      }
    ...
    public void endVisit(JMultiExpression x, Context ctx) {
      ...
      for (int i = 0; i < numRemovableExpressions(x); ++i) {
        JExpression expr = x.getExpressions().get(i);
        if (!expr.hasSideEffects()) {
          x.getExpressions().remove(i);
          --i;
          madeChanges();
          continue;
        }
        ...
        if (expr instanceof JMultiExpression) {
          x.getExpressions().remove(i);
          x.getExpressions().addAll(i, ((JMultiExpression) expr).getExpressions());
          i--;
          madeChanges();
          continue;
        }
    ...
    public void endVisit(JNewInstance x, Context ctx) {
      ...
      if (ignoringExpressionOutput.contains(x)) {
        ...
        JMultiExpression multi = new JMultiExpression(x.getSourceInfo());
        multi.getExpressions().addAll(x.getArgs());
        if (x.hasClinit()) {
          multi.getExpressions()
              .add(createClinitCall(x.getSourceInfo(), x.getTarget().getEnclosingType()));
        }
    ...
    private int numRemovableExpressions(JMultiExpression x) {
      if (ignoringExpressionOutput.contains(x)) {
        ...
        return x.getExpressions().size();
      } else {
        // The last expression cannot be removed.
        return x.getExpressions().size() - 1;
      }
    }
