...
public class FixAssignmentsToUnboxOrCast extends JModVisitor {
  ...
  private JBinaryOperation fixLhsCast(JBinaryOperation x) {
  ...
  private JBinaryOperation undoBoxIfNeed(JBinaryOperation x) {
  ...
  public void endVisit(JBinaryOperation x, Context ctx) {
    ...
    JBinaryOperation result = fixLhsCast(undoBoxIfNeed(x));

    if (result != x) {
      ctx.replaceMe(fixLhsCast(result));
    }
  }
}
