...
public class GenerateJavaScriptAST {
  ...
  private class GenerateJavaScriptVisitor extends GenerateJavaScriptLiterals {
    ...
    private void generateGwtOnLoad(JsFunction[] entryFuncs,
       ...
       * // Declare gwtOnLoad, but retain its previous value if any; this is kind of a hack
       * // var gwtOnLoad; should be enough here but HtmlUnit complains at first reference.
       * var gwtOnLoad = nullInUndefined(gwtOnLoad);
       * // Redefines gwtOnLoad to execute the previous gwtOnload and then execute the current
       * // module init functions.
       * ModUtils.createGwtOnLoad(rebindRegFunc, propRegFunc, gwtOnLoad, init1, init2,...)
       * </pre>
       */
      ...
      Iterables.addAll(arguments, Iterables.transform(
          Iterables.filter(Arrays.asList(entryFuncs), new Predicate<JsFunction>() {
            ...
            public boolean apply(JsFunction jsFunction) {
              return jsFunction != null && jsFunction != indexedFunctions.get("Impl.registerEntry");
            }
          }),
