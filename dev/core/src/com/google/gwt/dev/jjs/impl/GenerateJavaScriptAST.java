  ...
  private class GenerateJavaScriptVisitor extends JVisitor {
    ...
    private JsExpression dispatchToStatic(JMethodCall x, JMethod method, List<JsExpression> args) {
      JsNameRef methodName =
          method.isJsNative()
              ? createJsQualifier(method.getQualifiedExportName(), x.getSourceInfo())
    ...
    private JsExpression dispatchToSuper(JMethodCall x, JMethod method, List<JsExpression> args) {
      ...
      if (method.isConstructor()) {
        ...
        if (method.isJsNative()) {
          ...
          protoRef.setQualifier(createJsQualifier(superClass.getJsPrototype(), sourceInfo));
          methodNameRef = new JsNameRef(sourceInfo, method.getJsName());
