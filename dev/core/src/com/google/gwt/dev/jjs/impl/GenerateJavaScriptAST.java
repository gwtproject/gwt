...
public class GenerateJavaScriptAST {
  ...
  private class GenerateJavaScriptTransformer extends JTransformer<JsNode> {
     ...
     */
    private void declareClosureInterface(JInterfaceType x) {
      ...
      for (JMethod method : x.getMethods()) {
        if (method.isAbstract()) {
          ...
          generateVTableAssignment(method, polyName,
          ...
          if (method.isOrOverridesJsMethod()) {
            ...
            generateVTableAlias(method, exportedName);
          }
        }
      }
    }
     ...
     */
    private void generateInlinedDefineClass(JClassType x, JsName classVar) {
      JClassType superClass = x.getSuperClass();
    ...
    private void setupCastMapOnPrototype(JClassType x) {
    ...
    private void generateVTableAssignmentToJavaField(JClassType x, String javaField,
        JsExpression rhs) {
    ...
    private void generateVTables(JClassType x) {
      ...
      for (JMethod method : x.getMethods()) {
        ...
        } else if (closureCompilerFormatEnabled && method.isAbstract()) {
          // in closure mode, we add add a goog.abstractMethod for subtypes to @override
          generateVTableAssignment(method, polymorphicNames.get(method),
