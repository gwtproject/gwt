...
public class GenerateJavaScriptAST {
  ...
  private class GenerateJavaScriptTransformer extends JTransformer<JsNode> {
    ...
    private JsInteropExportsGenerator exportGenerator;
    ...
    private JsInteropExportsGenerator getExportGenerator() {
    ...
    private void generateExports() {
      ...
      for (JDeclaredType type : program.getDeclaredTypes()) {
        ...
        if (type.isJsType() && !type.getClassDisposition().isLocalType()) {
          ...
          getExportGenerator().exportType(type);
        }
        ...
        for (JMethod method : type.getMethods()) {
          ...
          getExportGenerator()
              .exportMember(method, names.get(method).makeRef(method.getSourceInfo()));
        }
        ...
        for (JField field : type.getFields()) {
          ...
          getExportGenerator().exportMember(field, names.get(field).makeRef(field.getSourceInfo()));
        }
      }
    }
