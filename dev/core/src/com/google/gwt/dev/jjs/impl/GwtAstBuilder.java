...
public class GwtAstBuilder {
  ...
  public static final char[][] JSARRAYLIKE =
      CharOperation.splitOn('.', "com.google.gwt.core.client.js.JsArrayLike".toCharArray());
  ...
  public static final char[] AT = "at".toCharArray();
  private ReferenceBinding jsArrayLike;
  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    public void endVisit(ForeachStatement x, BlockScope scope) {
      try {
        ...
        if (x.collectionVariable != null) {
          ...
          if (x.collection.resolvedType.isCompatibleWith(jsArrayLike)) {
            /**
             * <pre>
             * for (final T[] i$array = collection,
             *          int i$index = 0,
            ...
            MethodBinding lengthBinding = jsArrayLike.getExactMethod(LENGTH, NO_TYPES,
                cudScope);
            MethodBinding atBinding = jsArrayLike.getExactMethod(AT,
                INT_TYPE, cudScope);
            JMethod lengthMethod = typeMap.get(lengthBinding);
            JMethod atMethod = typeMap.get(atBinding);
            ...
            // T elementVar = i$array.at(i$index);
            elementDecl.initializer =
                new JMethodCall(info, new JLocalRef(info, arrayVar), atMethod,
                    new JLocalRef(info, indexVar));
            body.addStmt(0, elementDecl);
    ...
    private String synthesizeJsMagicMethodBodySource(MessageSend x, SourceInfo info,
        String javaScript, JMethod jsniMethod) {
  ...
  public List<JDeclaredType> process(CompilationUnitDeclaration cud, String sourceMapPath,
    ...
    jsArrayLike = cud.scope.environment().askForType(JSARRAYLIKE);
