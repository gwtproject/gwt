...
public class GwtAstBuilder {
  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    public void endVisit(TryStatement x, BlockScope scope) {
      try {
        ...
        if (x.resources.length > 0 ) {
          // Apply the source transformation described bellow to support try-with-resources.
        ...
        push(new JTryStatement(info, tryBlock, catchTypeLists, catchArgs, catchBlocks,
            finallyBlock));
      } catch (Throwable e) {
        throw translateException(x, e);
      }
    ...
    private JBlock normalizeTryWithResources(SourceInfo info, TryStatement x, JBlock tryBlock, BlockScope scope) {
      /**
      ...
      innerBlock.addStmt(new JTryStatement(info, tryBlock, catchTypes, catchArgs, catchBlocks,
            finallyBlock));
      return innerBlock;
    }
    ...
    private JStatement createCloseBlockFor(final SourceInfo info, final LocalDeclaration resource,
      ...
      // make try block
      JTryStatement tryBlock = new JTryStatement(info, innerTryBlock , catchTypes, catchArgs,
          catchBlocks, null);
