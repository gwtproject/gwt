  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    public void endVisit(ExplicitConstructorCall x, BlockScope scope) {
      try {
        ...
        ReferenceBinding constructorTargetClass =  x.binding.declaringClass;
        boolean isNestedTarget = JdtUtil.isInnerClass(constructorTargetClass);
        if (isNestedTarget) {
          if (x.isSuperAccess()) {
            processSuperCallThisArgs(constructorTargetClass, call, qualifier, x.qualification);
          } else {
            assert (x.qualification == null);
            processThisCallThisArgs(constructorTargetClass, call);
          }
        }
        call.addArgs(callArgs);
        if (isNestedTarget) {
          addCapturesToExplicitConstructorCall(constructorTargetClass, call);
        }
  ...
  private void createMembers(TypeDeclaration x) {
    ...
    try {
      ...
      if (type instanceof JClassType) {
        ...
        // TODO(rluble): we should assign synthetic fields here rather than in each constructor.
