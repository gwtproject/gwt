...
public class GwtAstBuilder {
  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    private JExpression box(JExpression original, int implicitConversion) {
      return box(original, implicitConversion, false);
    }

    private JExpression box(JExpression original, int implicitConversion, boolean doNotAutobox) {
      ...
      if (doNotAutobox) {
        return new JUnsafeTypeCoercion(original.getSourceInfo(), targetPrimitiveType, original);
      }
    ...
    private JExpression maybeBoxOrUnbox(
      ...
      if (requiresBoxing(implicitConversion)) {
        return box(original, implicitConversion, doNotAutobox);
      }
