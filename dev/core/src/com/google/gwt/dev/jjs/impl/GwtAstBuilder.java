...
public class GwtAstBuilder {
  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    private String nameForSyntheticArgument(SyntheticArgumentBinding synthArg) {
      return intern(intern(synthArg.actualOuterLocalVariable.name) + "_" + synthArg.resolvedPosition);
    }
    ...
    public void endVisit(ReferenceExpression x, BlockScope blockScope) {
      ...
      if (innerLambdaClass == null) {
        ...
        if (samMethod.getType() != JPrimitiveType.VOID) {
          int implicitConversion = x.implicitConversion;
          JExpression samExpression = samCall;
          if (x.binding.returnType.isBaseType() && x.binding.returnType != TypeBinding.VOID &&
              !(samMethod.getType() instanceof JPrimitiveType)) {
            samExpression = box(samCall, implicitConversion);
          }
          if (!x.binding.returnType.isBaseType() && x.binding.returnType != TypeBinding.VOID
              && samMethod.getType() instanceof JPrimitiveType) {
            samExpression = unbox(samCall, implicitConversion);
          }
    ...
    private JField createSyntheticField(SyntheticArgumentBinding arg, JDeclaredType enclosingType,
      ...
      String fieldName = nameForSyntheticArgument(arg);
