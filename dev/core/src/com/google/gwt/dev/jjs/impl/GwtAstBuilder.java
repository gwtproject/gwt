  ...
  class AstVisitor extends SafeASTVisitor {
    ...
    public void endVisit(ForStatement x, BlockScope scope) {
      try {
        ...
        JExpression incrementsExpression = null;
        if (incrementsExpressions.size() == 1) {
          incrementsExpression = incrementsExpressions.get(0);
        } else if (incrementsExpressions.size() > 1) {
          JMultiExpression incrementsMultiExpression = new JMultiExpression(info);
          incrementsMultiExpression.getExpressions().addAll(incrementsExpressions);
          incrementsExpression = incrementsMultiExpression;
        }
