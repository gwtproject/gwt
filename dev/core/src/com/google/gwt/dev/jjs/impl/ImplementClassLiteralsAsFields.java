...
public class ImplementClassLiteralsAsFields {
  ...
  public enum ClassLiteralFactoryMethod {
    CREATE_FOR_ENUM() {
      ...
      private JExpression getStandardMethodAsArg(SourceInfo info, JProgram program, JType type,
        ...
        if (enumType != type) {
          // The type is either an anonymous subclass that represents one of the enum values
          return JNullLiteral.INSTANCE;
        }

        // This is the base enum type not one of the anonymous classes that represent
        // enum values.
  ...
  private void execImpl() {
    if (!shouldOptimize) {
      // Create all class literals whether they are referenced or not.
