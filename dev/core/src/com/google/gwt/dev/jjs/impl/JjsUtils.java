  ...
  public static JConstructor getPrimaryConstructor(final JDeclaredType type) {
    List<JConstructor> delegatedSuperConstructors = FluentIterable
        ...
        .filter(new Predicate<JConstructor>() {
          @Override
          public boolean apply(JConstructor constructor) {
            JClassType delegatedConstructorEnclosingType =
                JjsUtils.getDelegatedThisOrSuperConstructor(constructor).getEnclosingType();
            return delegatedConstructorEnclosingType != type;
          }
        })
        .limit(2)
        .toList();
    if (delegatedSuperConstructors.size() == 1) {
      return Iterables.getOnlyElement(delegatedSuperConstructors);
    }
    return null;
  }
