...
public class JjsUtils {
  ...
  public static boolean isUnnecessarySyntheticOverride(JMethod method) {
    ...
    assert !method.isSyntheticOverride() || !method.exposesPackagePrivateMethod();

    if (!method.isSyntheticOverride()) {
      return false;
    }
    ...
    // A synthetic override is unnecessary iff it retains the same property name (polyname) as the
    // the concrete method it overrides.
