...
public class JsInteropRestrictionChecker extends JVisitor {
  ...
  public boolean visit(JDeclaredType x, Context ctx) {
    ...
    if (x.isJsFunction()) {
      checkJsFunctionType(x);
    } else if (x.isOrExtendsJsFunction() && x instanceof JClassType) {
      checkJsFunctionImplementationType(x);
  ...
  private void checkJsFunctionType(JDeclaredType type) {
  ...
  private void checkJsFunctionImplementationType(JDeclaredType type) {
    ...
    if (type.isJsType()) {
      logError(
          "'%s' cannot be both a JsFunction implementation and a JsType at the same time.", type);
    }
