...
public class JsInteropRestrictionChecker extends JVisitor {
  ...
  private void checkJsTypeMethod(JMethod method) {
    if (method.isSynthetic() && !method.isForwarding()) {
      ...
      // particular set of colliding method names) is exported. An exception to this situation is
      // when an accidental override forwarding method that occurs when a JsType interface starts
      // exposing a method in class B that is only ever implemented in its parent class A.
      return;
    }
