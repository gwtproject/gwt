...
public class JsInteropRestrictionChecker extends AbstractRestrictionChecker {
  ...
  private void checkSubclassOfNativeClass(JDeclaredType type) {
    ...
    for (JMethod method : type.getMethods()) {
      ...
      // Only look at synthetic overrides.
      for (JMethod overridenMethod : method.getOverriddenMethods()) {
        if (overridenMethod.getEnclosingType().isJavaLangObject()
            || overridenMethod.isSynthetic()
            || overridenMethod.getJsMemberType() != JsMemberType.NONE) {
          continue;
        }
        if (overridenMethod.getEnclosingType() instanceof JInterfaceType) {
          logError(
              type,
              "Class %s can not implement interface %s without explicitly implementing %s. "
                  + "Subclasses of native JsTypes cannot implement interfaces that declare equals, "
                  + "toString or hashCode through the native supertype.",
              getDescription(type),
              getDescription(overridenMethod.getEnclosingType()),
              getDescription(overridenMethod));
        }
      }
    }
  }
