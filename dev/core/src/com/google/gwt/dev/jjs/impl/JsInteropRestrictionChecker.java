...
public class JsInteropRestrictionChecker {
  ...
  private void checkJsMethod(JMethod method) {
    ...
    if (accessorType == JsPropertyAccessorType.GETTER) {
      ...
      if (Iterables.getFirst(membersByLocalName.getAllMembers(jsMemberName), null) != method) {
        // Only need to give the following errors for one of the conflicting members.
        return;
      }
      ...
      return;
    } else if (Iterables.getFirst(membersByLocalName.getAllMembers(jsMemberName), null) == method) {
  ...
  private void checkLocalNames(JDeclaredType type) {
    for (String jsName : membersByLocalName.getLocalNames()) {
      Collection<JMember> members = membersByLocalName.getRegularMembers(jsName);
  ...
  private void checkNameCollisionForPropertyAccessorAndRegular(
    ...
    boolean isLocalName = membersByLocalName.getRegularMembers(propertyName).size() > 0;
    ...
    if (isLocalName && isPropertyName) {
      logError("%s and %s can't both be named '%s' in type '%s'.",
          getDescription(membersByLocalName.getRegularMembers(propertyName).iterator().next()),
  ...
  private static class MembersByLocalName {
    ...
    MembersByLocalName(JDeclaredType type) {
      Set<JMethod> overridden = Sets.newHashSet();
      for (;type != null; type = type.getSuperClass())  {
        ...
        for (JMethod method : type.getMethods()) {
          ...
          overridden.addAll(method.getOverriddenMethods());
        }
      }
    }

    Collection<JMember> getAllMembers(String name) {
      return localMemberByMemberNames.get(name);
    }

    Collection<JMember> getRegularMembers(String name) {
      return Collections2.filter(localMemberByMemberNames.get(name), isRegularMethodPredicate);
    }

    Collection<JMethod> getSetters(String name) {
      return (Collection) Collections2.filter(
          localMemberByMemberNames.get(name), isSetterPredicate);
    }

    Collection<JMethod> getGetters(String name) {
      return (Collection) Collections2.filter(
          localMemberByMemberNames.get(name), isGetterPredicate);
    }
    ...
    private void collectLocalNames(JMethod method) {
      ...
      switch (method.getJsPropertyAccessorType()) {
        ...
        case SETTER :
        case NONE:
          localMemberByMemberNames.put(jsMemberName, method);
          break;
      }
    }
    ...
    private void collectLocalNames(JField field) {
      ...
      localMemberByMemberNames.put(field.getJsName(), field);
    }
  }
  ...
  private static Predicate<JMember> createAccessorPredicate(EnumSet types) {
