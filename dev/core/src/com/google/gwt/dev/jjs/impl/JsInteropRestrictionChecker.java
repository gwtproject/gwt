...
public class JsInteropRestrictionChecker extends JVisitor {
  ...
  private void checkJsTypeMethod(JMethod method) {
    ...
    } else if (method.getImmediateOrTransitiveJsPropertyType() == JsPropertyType.HAS) {
      // Has JS dispatch consumes no named slot on the prototype and so can not cause or suffer from
      // any collisions.
    } else if (method.getImmediateOrTransitiveJsPropertyType() == JsPropertyType.GET) {
      // If it's a getter.
    ...
    } else if (method.getImmediateOrTransitiveJsPropertyType() == JsPropertyType.SET) {
      // If it's a setter.
