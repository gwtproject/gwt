...
public class JsInteropRestrictionChecker extends AbstractRestrictionChecker {
  ...
  private void checkMember(JMember member, Multimap<String, JMember> localNames) {
    ...
    if (isInstanceJsMember(member)) {
      checkInstanceNameCollisions(localNames, member);
    }
  ...
  private void checkInstanceNameCollisions(Multimap<String, JMember> localNames, JMember member) {
  ...
  private void checkStaticNameCollisions(JMember member) {
    ...
    // Static property accessors are disallowed, if they get implemented add a check for
    // property consistency here..
