...
public class JsInteropRestrictionChecker {
  ...
  private void checkJsNative(JDeclaredType type) {
    ...
    if (type.getSuperClass() != null && type.getSuperClass() != jprogram.getTypeJavaLangObject() &&
        !type.getSuperClass().isJsNative()) {
    ...
    for (JInterfaceType interfaceType : type.getImplements()) {
      if (!interfaceType.isJsNative()) {
        logError("Native JsType '%s' can only implement native JsType interfaces.", type);
      }
    }
  }
  ...
  private void checkType(JDeclaredType type) {
    ...
    } else if (type.isJsType() && type instanceof JInterfaceType) {
      checkJsInterface(type);
    } else {
