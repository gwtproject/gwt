...
public class RedundantCastRemover {
   ...
   *
   * The types that are indicated by the test/condition expressions in the above cases are killed
   * after any definitions of the variable. So we have the following flow function:
   * indicatedType(out) = indicatedType(in) - def.
   * Since the analysis is based on AST but not CFG, the computation of definitions is conservative
   * but not so precise. For example, we encapsulate control flow statement 
   * ({if, while, for, switch} with {break, continue, etc.}) in one node, 
   * then the AST seems like a acyclic graph in which each node has only one entry and one exit,
   * like this: N1->N2->N3. Then we go through the AST once
   * and compute the indicated types at each statement.
   *
   * Since the 'break' and 'continue' are encapsulated, the analysis may not be precise in some
   * cases. For example, while(s0) {s1; if(s2){s3; break;} s4;}, definitions in s3 are reachable
   * to s4, which should not be.
   */
  public class FindAndRemoveRedundantCastVisitor extends ReplaceRedundantCastVisitor {
    public FindAndRemoveRedundantCastVisitor(OptimizerContext optimizerCtx) {
      super(optimizerCtx);
    }
    ...
    /**
     * At the first time we traverse a JForStatement, we ignore all the inferred types by its 
     * parent block due to its loop. After traverse the statement, we re-traverse it to remove
     * the leftover redundant cast checks.
     */
    @Override
    public void endVisit(JForStatement x, Context ctx) {
      endVisit((JStatement) x, ctx);
      ReplaceRedundantCastVisitor redundantCastReplacer =
          new ReplaceRedundantCastVisitor(getOptimizerContext());
      redundantCastReplacer.inferredTypesByVariable = inferredTypesByVariable;
      redundantCastReplacer.accept(x);
      for (int  i = 0; i < redundantCastReplacer.getNumMods(); i++) {
        madeChanges();
      }
    }
    ...
    public boolean visit(JForStatement x, Context ctx) {
      visit((JStatement) x, ctx);
      acceptWithInsertRemoveImmutable(x.getInitializers());
    ...
    public boolean visit(JIfStatement x, Context ctx) {
      visit((JStatement) x, ctx);
      traverseConditionalBranches(Arrays.asList(x.getIfExpr(), x.getThenStmt(), x.getElseStmt()));
      return false;
    }
  ...
  class ReplaceRedundantCastVisitor extends JChangeTrackingVisitor {
    public ReplaceRedundantCastVisitor(OptimizerContext optimizerCtx) {
      super(optimizerCtx);
    }
  ...
  public static OptimizerStats exec(OptimizerContext optimizerCtx) {
    Event optimizeEvent = SpeedTracerLogger.start(CompilerEventType.OPTIMIZE, "optimizer", NAME);
    OptimizerStats stats = new RedundantCastRemover().execImpl(optimizerCtx);
  ...
  public static OptimizerStats exec(JProgram program) {
    return exec(new FullOptimizerContext(program));
  }

  public OptimizerStats execImpl(OptimizerContext optimizerCtx) {
    OptimizerStats stats = new OptimizerStats(NAME);
    FindAndRemoveRedundantCastVisitor redundantCastRemover =
        new FindAndRemoveRedundantCastVisitor(optimizerCtx);
    Set<JMethod> modifiedMethods =
        optimizerCtx.getModifiedMethodsSince(optimizerCtx.getLastStepFor(NAME));
    optimizerCtx.traverse(redundantCastRemover, modifiedMethods);
