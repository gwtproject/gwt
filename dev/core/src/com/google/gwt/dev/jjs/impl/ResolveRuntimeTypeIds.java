...
/**
 * Replaces JRuntimeTypeReference nodes with the appropriate literal.
 */
public class ResolveRuntimeTypeIds {
  ...
  private class ReplaceRuntimeTypeReferencesVisitor extends JModVisitor {
    @Override
    public void endVisit(JRuntimeTypeReference x, Context ctx) {
      ctx.replaceMe(typeIdLiteralssByType.get(x.getReferredType()));
    }
  }

  public static Map<JType, JLiteral>  exec(JProgram program) {
    return new ResolveRuntimeTypeIds(program).execImpl();
  }

  private final JProgram program;

  private final Map<JType, JLiteral> typeIdLiteralssByType = Maps.newIdentityHashMap();

  private int nextAvailableId =  0;

  private ResolveRuntimeTypeIds(JProgram program) {
    this.program = program;
  }

  private void assignNextId(JType type) {
    if (typeIdLiteralssByType.containsKey(type)) {
      return;
    }
    ...
    typeIdLiteralssByType.put(type, JIntLiteral.get(id));
  }
