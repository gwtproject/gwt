...
public class RewriteConstructorCallsForUnboxedTypes extends JModVisitor {
  ...
  public RewriteConstructorCallsForUnboxedTypes(JProgram program) {
    ...
    JDeclaredType utilType = program.getIndexedType("Util");
    for (JMethod method : utilType.getMethods()) {
  ...
  public void endVisit(JNewInstance x, Context ctx) {
    ...
    if (!program.canBeUnboxedType(ctor.getEnclosingType())) {
      return;
    }

    // map BoxedType(args) -> Util.$createBoxedType(args)
    createMethod = jsniSig2Method.get("$create" + ctor.getJsniSignature(false, false));
  ...
  public void endVisit(JsniMethodRef x, Context ctx) {
    if (x.getTarget().isConstructor()
        && program.canBeUnboxedType(x.getTarget().getEnclosingType())) {
      JConstructor ctor = (JConstructor) x.getTarget();
      // map BoxedType(args) -> Util.$createBoxedType(args)
      JMethod createMethod = jsniSig2Method.get("$create" + ctor.getJsniSignature(false, false));
      assert createMethod != null;
