  ...
  public class TightenTypesVisitor extends JChangeTrackingVisitor {
    ...
    public void endVisit(JCastOperation x, Context ctx) {
      ...
      if ((!program.typeOracle.isInstantiatedType(toType) ||
          ...
          && toType != program.getTypeNull()) {
        // replace with a placeholder cast to NULL, unless it's already a cast to NULL
        ctx.replaceMe(new JCastOperation(x.getSourceInfo(), program.getTypeNull(), x.getExpr()));
        return;
      }
    ...
    public void endVisit(JConditional x, Context ctx) {
      ...
      JReferenceType resultType = program.strengthenAssignment(type,
          (JReferenceType) x.getThenExpr().getType(), (JReferenceType) x.getElseExpr().getType());
      if (type != resultType) {
      ...
      switch (analysisResult) {
        ...
        default:
          ...
          if (concreteType != null) {
            ctx.replaceMe(new JInstanceOf(x.getSourceInfo(), concreteType, x.getExpr()));
          }
          break;
      }
    }
    ...
    public void exit(JMethod x, Context ctx) {
      ...
      if (returnType == program.getTypeNull()) {
        return;
      }
     ...
     Iterable<JReferenceType> returnTypes = Iterables.concat(
      ...
      JReferenceType strengthenedType = program.strengthenAssignment(returnType, returnTypes);
    ...
    private void tighten(JVariable x) {
      ...
      Collection<JReferenceType> assignmentTypes = getAssigmentsIfValid(x);
      if (assignmentTypes == null) {
        return;
      }

      JReferenceType strengthenedType =
          program.strengthenAssignment(varType, Iterables.concat(assignmentTypes,
          JjsUtils.getExpressionTypes(paramUpRefs.get(x))));
    ...
    private Collection<JReferenceType> getAssigmentsIfValid(JVariable variable) {
      Collection<JExpression> assignedExpressions = TypeTightener.this.assignments.get(variable);
      ...
      for (JExpression expression : assignedExpressions) {
        ...
        if (!(expressionType instanceof JReferenceType)) {
          ...
          // such a case to replace the type of the lhs
          return null;
        }
