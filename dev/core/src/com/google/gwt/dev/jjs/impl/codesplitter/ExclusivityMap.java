...
class ExclusivityMap {
  ...
  private void compute(Collection<Fragment> exclusiveFragments,
    ...
    for (Fragment fragment : exclusiveFragments) {
      ...
      Set<JNode> nodesNotLiveInFragment = Sets.union(notLiveInFragment.getLiveFieldsAndMethods(),
              notLiveInFragment.getFieldsWritten());

      updateMap(fragment, fragmentForField, nodesNotLiveInFragment, liveFields);
      updateMap(fragment, fragmentForMethod, notLiveInFragment.getLiveFieldsAndMethods(),
          allLiveMethods);
      updateMap(fragment, fragmentForString, notLiveInFragment.getLiveStrings(), allLiveStrings);
