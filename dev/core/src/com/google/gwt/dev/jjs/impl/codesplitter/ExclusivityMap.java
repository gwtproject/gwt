...
class ExclusivityMap {
  ...
  /**
   * Updates the exclusivity map by recomputing exclusive methods for the changed fragments
   * and by removing deleted fragments
   */
  public void updateExclusivityMap(Collection<Fragment> changedFragments,
      ControlFlowAnalyzer completeCfa, Map<Fragment, ControlFlowAnalyzer> notLiveCfaByFragment) {

    // Recompute control flow analysis on the fragments that have changed.
    for (Fragment fragment : changedFragments) {
      assert fragment.isExclusive();
      ControlFlowAnalyzer allButOne = notLiveCfaByFragment.get(fragment);
      ...
      updateMethods(fragment, allButOne.getLiveFieldsAndMethods(),
          allMethods);
      updateStrings(fragment, allButOne.getLiveStrings(), completeCfa
          .getLiveStrings());
