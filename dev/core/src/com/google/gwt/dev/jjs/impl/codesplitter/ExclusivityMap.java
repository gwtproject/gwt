...
/**
 * A map from program atoms to the fragments (each fragment may contain more than one split point),
 * Maps an atom to the fragment, if any, that they are  exclusive to. Atoms not
...
class ExclusivityMap {
  ...
  public static final Fragment NOT_EXCLUSIVE = new Fragment(Fragment.Type.NOT_EXCLUSIVE) {
    ...
    public int getFragmentNumber() {
      throw makeUnsupportedException("getFragmentNumber");
    }

    @Override
    public List<JsStatement> getStatementsInFragment() {
      throw makeUnsupportedException("getStatementsInFragment");
    }

    @Override
    public void setStatementsInFragment(List<JsStatement> statementsInFragment) {
      throw makeUnsupportedException("setStatementsInFragment");
    }

    @Override
    public void addStatementsToFragment(List<JsStatement> statements) {
      throw makeUnsupportedException("addStatementsToFragment");
    }

    @Override
    public Set<JRunAsync> getSplitPoints() {
      throw makeUnsupportedException("getSplitPoints");
    }
    ...
    public void setFragmentNumber(int fragmentNumber) {
      throw makeUnsupportedException("setFragmentNumber");
    }
   ...
   * the specified fragment.
   */
  public void updateMethods(Fragment fragment, Set<?> notExclusiveAtoms, Iterable<JMethod> methods) {
   ...
   * the specified fragment.
   */
  public void updateTypes(Fragment fragment, Set<?> notExclusiveAtoms, Iterable<JDeclaredType> types) {
  ...
  private void fixUpLoadOrderDependenciesForClassLiterals(TreeLogger logger, JProgram jprogram) {
    ...
    while (!potentialClassLiteralFields.isEmpty()) {
      ...
      for (String string : stringsIn(initializer)) {
        ...
        if (!areFragmentsConsistent(classLiteralFragment, stringFrag)) {
          numFixups++;
      ...
      for (JClassLiteral superclassClassLiteral : classLiteralsIn(initializer)) {
        ...
        if (!areFragmentsConsistent(classLiteralFragment, superclassClassLiteralFragment)) {
    ...
    (new JVisitor() {
      ...
      public void endVisit(JField field, Context ctx) {
        if (field.getInitializer() instanceof JStringLiteral) {
          ...
          Fragment stringFrag = getFragment(fragmentForString, string);
          if (!areFragmentsConsistent(fieldFrag, stringFrag)) {
            numFixups[0]++;
            fragmentForString.put(string, NOT_EXCLUSIVE);
          }
        }
  ...
  private void fixUpLoadOrderDependenciesForTypes(TreeLogger logger, JProgram jprogram) {
    ...
    while (!typesToCheck.isEmpty()) {
      ...
      if (type.getSuperClass() != null) {
        ...
        if (!areFragmentsConsistent(typeFrag, supertypeFrag)) {
          numFixups++;
  ...
  /**
   * Returns true if atoms in otherFragment are visible from thisFragment.
   */
  private static boolean areFragmentsConsistent(Fragment thisFragment, Fragment otherFragment) {
    return thisFragment == otherFragment || !otherFragment.isExclusive();
  }

  private static <T> Fragment getFragment(Map<T, Fragment> map, T key) {
    Fragment value = map.get(key);
