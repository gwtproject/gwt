...
public class FragmentExtractor {
  ...
  /**
   * DefineClasss mark the existence of a class and associate a castMaps with the class's various
   * constructors. These multiple constructors are provided as JsNameRef varargs to the defineClass
  ...
  private JClassType vtableTypeAssigned(JsStatement stat) {
    ...
    if (expr.getExpression() instanceof JsInvocation) {
      ...
      if (func.getName() != jsprogram.getIndexedFunction("MetadataUtil.defineClass").getName()) {
        return null;
      }
