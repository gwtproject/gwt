...
class LiveAtomsByRunAsyncSets {
  ...
  private boolean countedAtoms = false;
  ...
  /**
   * Returns a list of lists of runAsyncs where {@link pairCount} lists are the result of greedily
  ...
  public List<List<JRunAsync>> mergeSimilarPairs(int pairCount) {
    ...
    while (fragmentRunAsyncLists.size() < pairCount && !subsetsDescending.isEmpty()) {
      ...
      // System.out.println("merging: " + largestSubset);
  ...
  public List<List<JRunAsync>> mergeSmallFragments(
      List<List<JRunAsync>> fragmentRunAsyncLists, int maxSize) {
    ...
    while (fragmentIterator.hasNext()) {
      ...
      if (!isFragmentTooLarge(fragmentRunAsyncs, maxSize)) {
    ...
    // TODO(rluble): Transform into logging
    // System.out.println("Merged small " + smallFragmentRunAsyncs);

    if (!smallFragmentRunAsyncs.isEmpty() && isFragmentTooLarge(smallFragmentRunAsyncs, maxSize)) {
      // Keep the fragment, if it was still small it would not be added to fragmentRunAsyncLists
      // hence its contents would have ended up in the leftovers.
  ...
  /**
   * Iteratively expand the initial sequence CFA with each runAsync and record the resulting live
   * atoms.
   */
  public void recordLiveSubsets(
  ...
  private <T> void accumulatePayloadSizes(Map<T, BitSet> liveSubsetsByAtom) {
    for (Map.Entry<T, BitSet> entry : liveSubsetsByAtom.entrySet()) {
      ...
      // Only record size for atoms that are in exactly two runAsyncs.
      // TODO(rluble): make this analysis more general purpose.
  ...
  /**
   * Accumulate payload sizes, but only once.
   */
  private void ensureAccumulatedPayloadSizes() {
    if (countedAtoms) {
      return;
    }
    countedAtoms = true;
  ...
  private boolean isFragmentTooLarge(List<JRunAsync> fragmentRunAsyncs, int maxSize) {
    ensureAccumulatedPayloadSizes();
    ...
    int size = payloadSizeBySubset.count(fragmentSubset);
    // Very inefficient, n^2 behavior.
    for (BitSet subset : payloadSizeBySubset) {
