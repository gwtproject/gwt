...
class MergeBySimilarityFragmentPartitionStrategy implements FragmentPartitionStrategy {
  ...
  public MergeBySimilarityFragmentPartitionStrategy(int targetNumberOfFragments, int minFragmentSize) {
  ...
  private List<List<JRunAsync>> partitionFragmentUsingEdgeGreedy(
    ...
    countAtomsSharedByTwoFragments(liveSplitPointMap.fields, matrix);
    countAtomsSharedByTwoFragments(liveSplitPointMap.methods, matrix);
    countAtomsSharedByTwoFragments(liveSplitPointMap.strings, matrix);
    countAtomsSharedByTwoFragments(liveSplitPointMap.types, matrix);
  ...
  private static Map<BitSet, Integer> atomCountBySubset = Maps.newHashMap();
  ...
  private static void printToNByCardinality(int n) {
    Map<Integer, PriorityQueue<Grouping>> sortedSubSets = Maps.newHashMap();
    for (BitSet bitSet : atomCountBySubset.keySet()) {
  ...
  private static <T> void countAtomsSharedByTwoFragments(Map<T, BitSet> livenessMap, int[][] matrix) {
    ...
    for (Map.Entry<T, BitSet> fieldLiveness : livenessMap.entrySet()) {
      ...
      {
        Integer count = atomCountBySubset.get(liveSplitPoints);
        count = count == null ? 0 : count;
        count++;
        atomCountBySubset.put(liveSplitPoints, count);
      }
