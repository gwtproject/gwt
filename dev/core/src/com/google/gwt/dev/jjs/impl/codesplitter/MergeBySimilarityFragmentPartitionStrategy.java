...
class MergeBySimilarityFragmentPartitionStrategy implements FragmentPartitionStrategy {
  ...
  public MergeBySimilarityFragmentPartitionStrategy(int targetNumberOfFragments, int maxSize) {
    this.targetNumberOfFragments = targetNumberOfFragments;
    this.maxSize = maxSize;
  }

  @Override
  public List<Fragment> partitionIntoFragments(
      ControlFlowAnalyzer initialSequenceCfa, Collection<JRunAsync> nonInitialRunAsyncs) {
    List<List<JRunAsync>> fragmentRunAsyncLists =
        mergeRunAsyncs(initialSequenceCfa, nonInitialRunAsyncs);
  ...
  private List<List<JRunAsync>> mergeRunAsyncs(
      ControlFlowAnalyzer initialSequenceCfa, Collection<JRunAsync> runAsyncs) {
    LiveAtomsByRunAsyncSets liveAtomsByRunAsyncSets = new LiveAtomsByRunAsyncSets();

    // Compute the under-approximate liveset map.
    liveAtomsByRunAsyncSets.recordLiveSubsets(initialSequenceCfa, runAsyncs);
    ...
    if (maxSize > 0) {
      liveAtomsByRunAsyncSets.mergeSmallFragments(fragmentRunAsyncLists, maxSize);
    }
