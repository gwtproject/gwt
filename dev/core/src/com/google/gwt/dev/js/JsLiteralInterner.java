...
public class JsLiteralInterner {
  ...
  private static class OccurenceCounter extends JsVisitor {
    ...
    public void endVisit(JsBinaryOperation x, JsContext ctx) {
      if (hasLhsLiteral(x)) {
        lhsLiterals.remove((JsLiteral) x.getArg1());
      }
    }
  ...
  private static class LiteralInterningVisitor extends JsModVisitor {
    ...
    public LiteralInterningVisitor(JProgram program, JsScope scope,
        Multiset<JsLiteral> occurrenceMap) {
    ...
    public boolean visit(JsObjectLiteral x, JsContext ctx) {
      maybeInternLiteral(x, ctx);
      // TODO(rluble): Literals on the right hand side of property initilizers should be
      // considered for interning also.
      return false;
    }
    ...
    private void maybeInternLiteral(JsLiteral x, JsContext ctx) {
