...
public class JsNamespaceChooser {
  ...
  private static String findPackage(JDeclaredType type) {
    ...
    int lastDot = dollar == -1 ? typeName.lastIndexOf('.') : typeName.lastIndexOf('.', dollar);
    if (lastDot == -1) {
      return null; // not found
    }
  ...
  /**
   * A compiler pass that moves all function and global variable definitions to the
   * correct namespace and fixes all name references.
   */
  private static class NameFixer extends JsModVisitor {

    /**
     * Replace "function foo()" with "name.foo = function()".
     */
    @Override
    public void endVisit(JsFunction x, JsContext ctx) {
      ...
      if (name != null && name.getNamespace() != null) {
        ...
        JsBinaryOperation assign =
            new JsBinaryOperation(x.getSourceInfo(), JsBinaryOperator.ASG, newName, x);
        x.setName(null);
        ctx.replaceMe(assign);
      }
    }
