...
public class JsReportGenerationVisitor extends
  ...
  protected <T extends JsVisitable> T generateAndBill(T node, JsName nameToBillTo) {
    ...
    if (node instanceof JsBlock) {
      ...
      if ((node instanceof JsExpression) &&
          (parent instanceof JsStatement) &&
          isLoop((JsStatement) parent)) {
        // Instrument the expression so we can stop there in a loop.
        willReportRange = true;
      } else {
        SourceInfo info = ((JsNode) node).getSourceInfo();
        if (!surroundsInJavaSource(parent.getSourceInfo(), info)) {
          // Instrument the expression of an inlined function.
          willReportRange = true;
        }
      }
    }
  ...
  private boolean isLoop(JsStatement node) {
    return (node instanceof JsDoWhile) || (node instanceof JsFor) || (node instanceof JsForIn) ||
        (node instanceof JsSwitch) || (node instanceof JsWhile);
  }
   ...
   * the child.
   */
  private boolean surroundsInJavaSource(SourceInfo parent, SourceInfo child) {
