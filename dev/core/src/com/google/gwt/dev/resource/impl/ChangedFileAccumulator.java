...
public class ChangedFileAccumulator {
      ...
      while (true) {
        ...
        for (WatchEvent<?> watchEvent : watchKey.pollEvents()) {
          ...
          if (eventKind == StandardWatchEventKinds.ENTRY_CREATE && Files.isDirectory(changedPath)) {
            // A new directory and some contained files can be created faster than watches can be
            // attached to new directories. So it is necessary to look for files in what are
            // believed to be "newly created" directories and consider those files changed.
            recursivelyRegisterListeners(changedPath, true /* considerPreexistingFilesChanged */);
          }
   ...
   * were requested.
   */
  public List<File> getAndClearModifiedFiles() {
    if (failed) {
