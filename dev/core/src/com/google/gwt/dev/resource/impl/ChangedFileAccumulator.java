...
public class ChangedFileAccumulator {
  ...
  /**
   * An exception that indicates that this changed file accumulator has failed and is no longer
   * valid.
   */
  public static class ChangeAccumulationFailedException extends Exception {
    public ChangeAccumulationFailedException(String message) {
      super(message);
    }
  }

  /**
   * A runnable that polls for watch events (in a blocking manner). File modifications are recorded
          ...
          synchronized (changedFiles) {
            ...
            for (WatchEvent<?> watchEvent : watchKey.pollEvents()) {
              ...
              if (eventKind == StandardWatchEventKinds.OVERFLOW) {
                setFailed("Changes occured faster than they could be recorded.");
                return;
              }
   ...
   * were requested.
   */
  public List<File> getAndClearModifiedFiles() throws ChangeAccumulationFailedException {
    if (isFailed()) {
      throw new ChangeAccumulationFailedException(failureMessage);
    }

    synchronized (changedFiles) {
      List<File> sortedModifiedFiles = Lists.newArrayList(changedFiles);
      changedFiles.clear();
      Collections.sort(sortedModifiedFiles);
      return sortedModifiedFiles;
    }
  }
  ...
  private void recursivelyRegisterListeners(final Path directory,
      final boolean considerPreexistingFilesChanged) {
    try {
  ...
  private void setFailed(String failureMessage) {
