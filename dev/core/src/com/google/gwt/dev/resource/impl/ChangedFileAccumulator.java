...
public class ChangedFileAccumulator {
  ...
  private void removePath(final Path parent) {
    changedFiles.add(parent.toFile());
    WatchKey removed = pathsByWatchKey.inverse().remove(parent);
    if (removed != null) {
       removed.cancel();
    }
    for (Path child : pathsByParentPath.removeAll(parent)) {
      removePath(child);
    }
  }
