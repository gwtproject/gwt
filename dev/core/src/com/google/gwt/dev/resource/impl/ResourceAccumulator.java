...
public class ResourceAccumulator {
  ...
  private Multimap<Path, Path> pathsByPath;
  ...
  /**
   * Full refresh clears existing resources and watchers and do a clean refresh.
   */
  private void fullRefresh() throws IOException {
    resolutionsByResource = Maps.newIdentityHashMap();
    pathsByPath = ArrayListMultimap.create();
  ...
  public void shutdown() {
    try {
      // watchService field is not cleared so any attempt to use this class will fail.
      stopWatchService();
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private void onNewDirectory(Path directory) throws IOException {
    ...
    try (DirectoryStream<Path> stream = Files.newDirectoryStream(directory)) {
      for (Path child : stream) {
        pathsByPath.put(directory, child);
        onNewPath(child);
      }
    }
  }
  ...
  private void onRemovedPath(Path path) {
    ...
    for (Path child : pathsByPath.get(path)) {
      onRemovedPath(child);
    }
  }
