  ...
  public void invokeJavascript(CompilingClassLoader ccl, JsValueOOPHM jsthis,
    ...
    try {
      ...
      if (msg.isException()) {
        ...
        if (returnValue.isJsObject() &&
            (returnJsValue.getTypeString().contains("TypeError") ||
                returnJsValue.getTypeString().contains("ReferenceError"))) {
          // Assume that NPE manifest in JavaScript as TypeError or ReferenceError.
          RuntimeException npe = new NullPointerException();
          npe.initCause(exception);
          npe.setStackTrace(exception.getStackTrace());
          exception = npe;
        }
        throw exception;
      }
