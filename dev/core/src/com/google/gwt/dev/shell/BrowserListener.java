...
public class BrowserListener implements CodeServerListener {
   ...
   * @param handler
   */
  public BrowserListener(TreeLogger logger, HostedModeOptions options,
    ...
    try {
      ...
      listenSocket.bind(new InetSocketAddress(address, options.getPort()));
        ...
        public void run() {
          while (true) {
            try {
              ...
              /*
               * This object is special-cased by the SessionHandler, used for methods needed by the
               * client like hasMethod/hasProperty/etc. handler is used for this object just to make
               * sure it doesn't conflict with some real object exposed to the client.
               */
  ...
  public URL processUrl(String url) throws UnableToCompleteException {
    /*
     * TODO(jat): properly support launching arbitrary browsers -- need some UI API tweaks to
     * support that.
     */
    URL parsedUrl = null;
    try {
  ...
  @Override
  public synchronized void produceBrowserOutput(StandardLinkerContext linkerStack,
