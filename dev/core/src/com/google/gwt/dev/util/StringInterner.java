...
public final class StringInterner {
  ...
  public String intern(String original) {
    AtomicReferenceArray<Entry> localTable = table;
    int index;
    Entry head;
    ValueEntry newEntry;
    ValueEntry lastHead = null;

    do {
      index = original.hashCode() & getMask(localTable);
      head = localTable.get(index);
      
      // Look for a forward
      if (head instanceof ForwardingEntry) {
        // Some other thread has rehashed this bucket
        localTable = ((ForwardingEntry) head).forward;
        newEntry = null;
        lastHead = null;
        continue;
      }
      ...
      for (ValueEntry current = vHead; current != lastHead; current = current.next) {
      ...
      newEntry = new ValueEntry(original, vHead);
      lastHead = vHead;
    } while ((newEntry == null) || !localTable.compareAndSet(index, head, newEntry));
