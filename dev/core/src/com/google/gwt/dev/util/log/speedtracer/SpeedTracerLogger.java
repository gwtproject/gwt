...
public final class SpeedTracerLogger {
  ...
  private static final boolean LOG_PROCESS_CPU_TIME =
  ...
  private static final boolean JSNI_CALL_LOGGING_ENABLED =
      !getBooleanProperty("gwt.speedtracer.disableJsniLogging");

  static {
    // verify configuration
    if (LOG_PROCESS_CPU_TIME && LOG_THREAD_CPU_TIME) {
  ...
  public class Event {
    ...
    public long getDurationNanos() {
      return LOG_PROCESS_CPU_TIME ? processCpuDurationNanos : (LOG_THREAD_CPU_TIME
    ...
    public long getStartTimeNanos() {
      return LOG_PROCESS_CPU_TIME ? processCpuStartTimeNanos : (LOG_THREAD_CPU_TIME
  ...
  public static boolean jsniCallLoggingEnabled() {
    return JSNI_CALL_LOGGING_ENABLED;
  }
  ...
  private final ProcessNormalizedTimeKeeper processCpuTimeKeeper =
              (LOG_PROCESS_CPU_TIME) ? new ProcessNormalizedTimeKeeper() : null;

  private final ThreadNormalizedTimeKeeper threadCpuTimeKeeper =
              (LOG_THREAD_CPU_TIME) ? new ThreadNormalizedTimeKeeper() : null;
  ...
  private SpeedTracerLogger() {
    fileLoggingEnabled = LOG_FILE != null;
    ...
    if (enabled) {
      if (fileLoggingEnabled) {
        ...
        if (DEFAULT_FORMAT_STRING != null) {
          for (Format value : Format.values()) {
            if (value.name().toLowerCase().equals(DEFAULT_FORMAT_STRING.toLowerCase())) {
              format = value;
              break;
            }
          }
  ...
  private BlockingQueue<Event> openDefaultLogWriter() {
    ...
    if (enabled) {
      try {
        ...
        return openLogWriter(writer, LOG_FILE);
      } catch (IOException e) {
        LOG.log(Level.SEVERE, "Unable to open gwt.speedtracerlog '" + LOG_FILE + "'", e);
      }
    }
    return null;
  }
