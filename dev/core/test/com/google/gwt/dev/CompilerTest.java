...
public class CompilerTest extends ArgProcessorTestBase {
  ...
  public void testPerFileRecompile__devirtualizeUnchangedJso() throws UnableToCompleteException,
      IOException, InterruptedException {
    // Tests that a JSO calls through interfaces are correctly devirtualizes..
  ...
  private void checkRecompiledModifiedApp(CompilerOptions compilerOptions, String moduleName,
    ...
    // If a resource was changed between the original compile and the relink compile.
    // If the compile is correct then the output JS will have changed otherwise if recompiling with
    // an unchanged resorce outputs should be the same.
