...
public class EnumOrdinalizerTest extends OptimizerTestBase {
  ...
  protected boolean optimizeMethod(JProgram program, JMethod method) {
    ...
    JJSOptions optionsMock = mock(JJSOptions.class);
    when(optionsMock.isCastCheckingDisabled()).thenReturn(false);
    CompilerContext compilerContext = mock(CompilerContext.class);
    when(compilerContext.getJProgram()).thenReturn(program);
    when(compilerContext.getOptions()).thenReturn (optionsMock);
    ...
    if (runCastNormalizer) {
      CompilerPass.exec(compilerContext, CastNormalizer.class);
    }
    if (runEqualityNormalizer) {
      CompilerPass.exec(compilerContext, EqualityNormalizer.class);
    }
