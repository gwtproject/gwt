...
public class Java8AstTest extends JJSTestBase {
  ...
  public void testCompileLambdaCaptureLocal() throws Exception {
    ...
    assertEquals("{this.x=x;}", formatSource(ctor.getBody().toSource()));
    // should extends test.Lambda
    assertTrue(lambdaInnerClass.getImplements().contains(program.getFromTypeMap("test.Lambda")));
    ...
    JMethod samMethod = findMethod(lambdaInnerClass, "run");
    assertEquals(
        "public final Object run(int arg0,int arg1){return EntryPoint.lambda$0(this.x,arg0,arg1);}",
        formatSource(samMethod.toSource()));
  }
  ...
  public void testCompileLambdaCaptureLocalAndField() throws Exception {
    ...
    assertEquals("{this.$$outer=$$outer;this.x=x;}", formatSource(ctor.getBody().toSource()));
    // should extends test.Lambda
    assertTrue(lambdaInnerClass.getImplements().contains(program.getFromTypeMap("test.Lambda")));
    ...
    JMethod samMethod = findMethod(lambdaInnerClass, "run");
    assertEquals(
        "public final Object run(int arg0,int arg1){return this.$$outer.lambda$0(this.x,arg0,arg1);}",
        formatSource(samMethod.toSource()));
  }
  ...
  public void testCompileLambdaCaptureOuterInnerField() throws Exception {
    ...
    assertEquals("{return this.this$01.y+this.y+a+b;}", formatSource(lambdaMethod.getBody().toSource()));
    // created by GwtAstBuilder
    ...
    // should contain assignment statement of ctor params to field
    assertEquals("{this.$$outer=$$outer;}", formatSource(ctor.getBody().toSource()));
    // should extends test.Lambda
    assertTrue(lambdaInnerClass.getImplements().contains(program.getFromTypeMap("test.Lambda")));
    ...
    JMethod samMethod = findMethod(lambdaInnerClass, "run");
    assertEquals(
        "public final Object run(int arg0,int arg1){return this.$$outer.lambda$0(arg0,arg1);}",
        formatSource(samMethod.toSource()));
  }
  ...
  public void testCompileStaticReferenceBinding() throws Exception {
    ...
    assertEqualBlock(
        "(new AcceptsLambda()).accept(new private$test_EntryPoint$foo__IILjava_lang_Integer_2$Type());",
        lambda
    );
    JProgram program = compileSnippet("void", lambda, false);
    // created by JDT, should exist
    ...
    // created by GwtAstBuilder
    JClassType lambdaInnerClass = (JClassType) getType(program, "test.private$test_EntryPoint$foo__IILjava_lang_Integer_2$Type");
    assertNotNull(lambdaInnerClass);

    // should have constructor taking this and x
    JMethod ctor = findMethod(lambdaInnerClass,"private$test_EntryPoint$foo__IILjava_lang_Integer_2$Type");
    assertTrue(ctor instanceof JConstructor);
    // no ctor args
    assertEquals(0, ctor.getParams().size());
  ...
  public void testCompileInstanceReferenceBinding() throws Exception {
    ...
    assertEqualBlock(
        "(new AcceptsLambda()).accept(new private$test_EntryPoint$foo__IILjava_lang_Integer_2$Type(this));",
        lambda
    );
    JProgram program = compileSnippet("void", lambda, false);
    // created by JDT, should exist
    ...
    JMethod samMethod = findMethod(lambdaInnerClass, "run");
    assertEquals(
        "public final Object run(int arg0,int arg1){return this.$$outer.foo(arg0,arg1);}",
        formatSource(samMethod.toSource()));
  }
  ...
  public void testCompileImplicitQualifierReferenceBinding() throws Exception {
    ...
    assertEqualBlock(
        "(new AcceptsLambda()).accept2(new private$java_lang_String$equalsIgnoreCase__Ljava_lang_String_2Z$Type());",
        lambda
    );
    JProgram program = compileSnippet("void", lambda, false);

    // created by GwtAstBuilder
    JClassType lambdaInnerClass = (JClassType) getType(program, "java.lang.private$java_lang_String$equalsIgnoreCase__Ljava_lang_String_2Z$Type");
    assertNotNull(lambdaInnerClass);

    // should have constructor taking this and x
    JMethod ctor = findMethod(lambdaInnerClass,"private$java_lang_String$equalsIgnoreCase__Ljava_lang_String_2Z$Type");
    assertTrue(ctor instanceof JConstructor);
    // no instance capture
    assertEquals(0, ctor.getParams().size());
  ...
  public void testCompileConstructorReferenceBinding() throws Exception {
    ...
    assertEqualBlock(
        "(new AcceptsLambda()).accept(new private$test_Pojo$Pojo__IIV$Type());",
        lambda
    );
    JProgram program = compileSnippet("void", lambda, false);

    // created by GwtAstBuilder
    JClassType lambdaInnerClass = (JClassType) getType(program, "test.private$test_Pojo$Pojo__IIV$Type");
    assertNotNull(lambdaInnerClass);

    // should have constructor taking this and x
    JMethod ctor = findMethod(lambdaInnerClass,"private$test_Pojo$Pojo__IIV$Type");
    assertTrue(ctor instanceof JConstructor);
    // no instance capture
    assertEquals(0, ctor.getParams().size());
  ...
  public void testDefaultInterfaceMethod() throws Exception {
    ...
    // created by GwtAstBuilder
    JInterfaceType lambdaInnerClass = (JInterfaceType) getType(program, "test.DefaultInterface");
    // should have an actual method with body on it
    JMethod defaultMethod = findMethod(lambdaInnerClass, "method2");
  ...
  private JMethod getMethod(JProgram program, String name) throws UnableToCompleteException  {
    return findMethod(program, name);
  }

  private JReferenceType getType(JProgram program, String name) throws UnableToCompleteException  {
