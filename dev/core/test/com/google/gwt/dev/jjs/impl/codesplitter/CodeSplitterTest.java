...
//  Fragment #1
    ...
    assertMethodInFragment("EntryPoint$Foo", 0, JPrimitiveType.INT);
    ...
    assertMethodNotInFragment("EntryPoint$Foo", 0, stringType);
    ...
    assertMethodInFragment("EntryPoint$Foo", 1, stringType);
    ...
    assertMethodNotInFragment("EntryPoint$Foo", 1, JPrimitiveType.INT);

    // Verify fragment 2 (leftovers)
    assertNotInFragment("functionAllocateFooInt", 2);
    assertNotInFragment("functionAllocateFooString", 2);

    assertMethodNotInFragment("EntryPoint$Foo", 2, JPrimitiveType.INT);
    assertMethodNotInFragment("EntryPoint$Foo", 2, stringType);
  ...
  private void assertMethodInFragment(String functionName, int expectedFragmentNumber,
      JType... args) {
    Set<Integer> fragments = Sets.newHashSet();
    for (int fragmentNumber = 0; fragmentNumber < jsProgram.getFragmentCount(); fragmentNumber++) {
      JsBlock fragment = jsProgram.getFragmentBlock(fragmentNumber);
  ...
  private void assertMethodNotInFragment(String functionName, int fragmentNum, JType...
      args) {
    JsBlock fragment = jsProgram.getFragmentBlock(fragmentNum);
    assertFalse("function " + functionName + " should not be in fragment " + fragmentNum,
