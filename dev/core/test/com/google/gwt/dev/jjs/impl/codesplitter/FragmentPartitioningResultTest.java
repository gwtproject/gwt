...
/**
 * Tests {@link FragmentPartitioningResult#getSafeSharedFragment(int, int)}.
 */
public class FragmentPartitioningResultTest extends TestCase {

  private final int NUM_RUNASYCS = 10;
  private final JRunAsync[] runAsyncs = new JRunAsync[NUM_RUNASYCS + 1];

  @Override
  public void setUp() {
    // Create some runAsyncs.
    for (int i=1; i <= NUM_RUNASYCS; i++) {
  ...
  public void testBasics() {
    ...
    final int LEFTOVERS_ID = NUM_RUNASYCS + 1;
    ...
    FragmentPartitioningResult fragmentPartitioningResult =
        new FragmentPartitioningResult(fragments, NUM_RUNASYCS);

    // The closest common ancestor is itself.
    assertEquals(0, fragmentPartitioningResult.getSafeSharedFragment(0, 0));
    assertEquals(1, fragmentPartitioningResult.getSafeSharedFragment(1, 1));
    assertEquals(3, fragmentPartitioningResult.getSafeSharedFragment(3, 3));
    assertEquals(6, fragmentPartitioningResult.getSafeSharedFragment(6, 6));
    assertEquals(11, fragmentPartitioningResult.getSafeSharedFragment(11, 11));

    // Lowest number in the initial sequence is the closest common ancestor.
    assertEquals(0, fragmentPartitioningResult.getSafeSharedFragment(11, 0));
    assertEquals(0, fragmentPartitioningResult.getSafeSharedFragment(10, 0));
    assertEquals(0, fragmentPartitioningResult.getSafeSharedFragment(3, 0));

    // Initial sequence fragments load before all others
    assertEquals(3, fragmentPartitioningResult.getSafeSharedFragment(3, 10));
    assertEquals(3, fragmentPartitioningResult.getSafeSharedFragment(10, 3));

    // Earlier initial sequence fragments load before the others
    assertEquals(1, fragmentPartitioningResult.getSafeSharedFragment(1, 2));
    assertEquals(1, fragmentPartitioningResult.getSafeSharedFragment(2, 1));
    assertEquals(2, fragmentPartitioningResult.getSafeSharedFragment(2, 3));

    // For non-equal exclusive fragments, leftovers is the common predecessor
    assertEquals(LEFTOVERS_ID, fragmentPartitioningResult.getSafeSharedFragment(6, 8));
    assertEquals(LEFTOVERS_ID, fragmentPartitioningResult.getSafeSharedFragment(9, 8));

    // Leftovers is before any exclusive
    assertEquals(LEFTOVERS_ID, fragmentPartitioningResult.getSafeSharedFragment(LEFTOVERS_ID, 8));

    // Inintial are before leftover
    assertEquals(1, fragmentPartitioningResult.getSafeSharedFragment(LEFTOVERS_ID, 1));
  }
  ...
  public void testWithEmptyInitial() {
    ...
    // Inintial are before leftover
    assertEquals(0, fragmentPartitioningResult.getSafeSharedFragment(LEFTOVERS_ID, 0));
  }
}
