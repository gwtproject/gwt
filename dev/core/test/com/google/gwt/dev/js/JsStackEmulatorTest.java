...
public class JsStackEmulatorTest extends FullCompileTestBase {
  ...
  public void testThrowWithInlineMethodCall() throws Exception {
    ...
    JsProgram program = compileClass(
        ...
        "  private static String message() { return thing.toString(); }",
        "  public static void onModuleLoad() {",
    ...
    checkOnModuleLoad(program, "function onModuleLoad(){" +
        "var stackIndex;$stack[stackIndex=++$stackDepth]=onModuleLoad;" +
        "$location[stackIndex]='EntryPoint.java:'+'5',$clinit_EntryPoint();" +
        "throw $location[stackIndex]='EntryPoint.java:'+'6'," +
        "new ($location[stackIndex]='EntryPoint.java:'+'6'," +
        "RuntimeException)(($tmp=($location[stackIndex]='EntryPoint.java:'+'4',thing).toString$()," +
        "$location[stackIndex]='EntryPoint.java:'+'4',$tmp))" +
        "}");
  }
  ...
  private JsProgram compileClass(String... lines) throws UnableToCompleteException {
    ...
    Pair<JavaToJavaScriptMap,Set<JsNode>> pair = GenerateJavaScriptAST.exec(
        jProgram, jsProgram, context, typeIdsByType, symbolTable,
        properties);
    JavaToJavaScriptMap jjsmap = pair.getLeft();
    Set<JsNode> inlinable = pair.getRight();
