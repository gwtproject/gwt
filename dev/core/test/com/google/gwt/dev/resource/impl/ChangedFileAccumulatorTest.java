...
import com.google.gwt.dev.resource.impl.ChangedFileAccumulator.ChangeAccumulationFailedException;
...
public class ChangedFileAccumulatorTest extends TestCase {
  ...
  public void testAddFile() throws IOException, InterruptedException,
      ChangeAccumulationFailedException {
    ...
    assertTrue(changedFileAccumulator.getAndClearModifiedFiles().isEmpty());

    createFileIn("New.java", subDirectory);
    waitForFileEvents();

    List<File> modifiedFiles = changedFileAccumulator.getAndClearModifiedFiles();
  ...
  public void testModifyRepeatedly() throws IOException, InterruptedException,
      ChangeAccumulationFailedException {
    File rootDirectory = Files.createTempDir();
  ...
  public void testMultipleListeners() throws IOException, InterruptedException,
      ChangeAccumulationFailedException {
    File rootDirectory = Files.createTempDir();
    File subDirectory = createDirectoryIn("subdir", rootDirectory);
    ...
    assertTrue(changedFileAccumulator1.getAndClearModifiedFiles().isEmpty());
    assertTrue(changedFileAccumulator2.getAndClearModifiedFiles().isEmpty());

    createFileIn("New.java", subDirectory);
    waitForFileEvents();

    List<File> modifiedFiles1 = changedFileAccumulator1.getAndClearModifiedFiles();
    assertEquals(1, modifiedFiles1.size());
    assertTrue(modifiedFiles1.get(0).getPath().endsWith("New.java"));

    List<File> modifiedFiles2 = changedFileAccumulator2.getAndClearModifiedFiles();
  ...
  public void testRenameFile() throws IOException, InterruptedException,
      ChangeAccumulationFailedException {
    File rootDirectory = Files.createTempDir();
    File subDirectory = createDirectoryIn("subdir", rootDirectory);
  ...
  public void testSymlinks() throws IOException, InterruptedException,
      ChangeAccumulationFailedException {
