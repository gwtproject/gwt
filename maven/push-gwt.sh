...
gwtPathDefault=${GWT_DIST_FILE:=$(ls -r1 ${gwtTrunk}/build/dist/gwt-*.zip 2>/dev/null | head -1)}
...
gwtVersionDefault=${GWT_VERSION:=$(expr "$gwtPathDefault" : '.*\('$VERSION_REGEX'\).zip')}
