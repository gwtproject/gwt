...
final class TypeSimplifier {
  ...
  private final Map<String, TypeSimplifier.Spelling> imports;
  ...
  SortedSet<String> typesToImport() {
    ...
    for (Map.Entry<String, TypeSimplifier.Spelling> entry : imports.entrySet()) {
  ...
  String simplify(TypeMirror type) {
    return type.accept(new TypeSimplifier.ToStringTypeVisitor(), new StringBuilder()).toString();
  }
   ...
   */
  private static Map<String, TypeSimplifier.Spelling> findImports(
      Types typeUtil, String packageName, Set<TypeMirror> referenced) {
    Map<String, TypeSimplifier.Spelling> imports = new HashMap<String, TypeSimplifier.Spelling>();
    ...
    for (TypeMirror type : referenced) {
      ...
      imports.put(fullName, new TypeSimplifier.Spelling(spelling, importIt));
    }
    return imports;
  }
  ...
  private static Set<TypeMirror> referencedClassTypes(Types typeUtil, Set<TypeMirror> types) {
    ...
    TypeVisitor<Void, Void> typeVisitor = new TypeSimplifier.ReferencedClassTypeVisitor(typeUtil, referenced);
