...
public final class Impl {
  ...
  public static native void registerWindowOnError(boolean reportIfNoHandler) /*-{
    var origWindowHandler = $wnd.onerror;
    var origFrameHandler = window.onerror;
    
    var errorHandler = function(msg, url, line, column, error) {
    ...
    var frameHandler = function(msg, url, line, column, error) {
      errorHandler(msg, url, line, column, error);
      if (origFrameHandler) {
        origFrameHandler(msg, url, line, column, error);
      }
      return false;
    };

    var windowHandler = function(msg, url, line, column, error) {
      errorHandler(msg, url, line, column, error);
      if (origWindowHandler) {
        origWindowHandler(msg, url, line, column, error);
      }
      return false;
    };
    ...
    if (!(origFrameHandler && reportIfNoHandler)) {
      window.onerror = windowHandler;
    }
    if (!(origWindowHandler && reportIfNoHandler)) {
      $wnd.onerror = windowHandler;
    }
  }-*/;
