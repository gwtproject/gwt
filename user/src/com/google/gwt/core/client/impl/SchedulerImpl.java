...
public class SchedulerImpl extends Scheduler {
  ...
  static final class Task extends JavaScriptObject {
     ...
     */
    public int executeRepeating(Duration duration, double goalElapsedTime, int startMilli) {
      ...
      if (repeat) {
        ...
        if (avgTimeStack.length() > 0) {
          ...
          guessCount = (int) ((goalElapsedTime - startMilli)/ slowestAvg);
        }
  ...
  private JsArray<Task> runRepeatingTasks(JsArray<Task> tasks) {
    ...
    if (length < 7) {   // Predictive scheduler performs worse as more tasks are added.
                        // So is only run if fewer than 7 tasks are currently scheduled.
