...
public class CurrencyListGenerator extends Generator {
  ...
  private String generateLocaleTree(TreeLogger logger, GeneratorContext context,
    ...
    for (int i = searchList.size(); i-- > 0;) {
      ...
      LocalizedProperties newExtra = getProperties(logger, CURRENCY_EXTRA_PREFIX, search, context);
      if (newExtra != null) {
        currencyExtra = newExtra;
      }
      Map<String, String> currencyData = getCurrencyData(logger, search, context);
      ...
      String defCurrencyCode = getDefaultCurrency(logger, search, context);
  ...
  private Map<String, String> getCurrencyData(TreeLogger logger, GwtLocale locale,
    ...
    LocalizedProperties currencyData =
        getProperties(logger, CURRENCY_DATA_PREFIX, locale, context);
    if (currencyData == null) {
   ...
   */
  private String getDefaultCurrency(TreeLogger logger, GwtLocale locale, GeneratorContext context) {
    String defCurrencyCode = null;
    LocalizedProperties numberConstants =
        getProperties(logger, NUMBER_CONSTANTS_PREFIX, locale, context);
   ...
   * @param genCtx with which to locate resources
   *
  ...
  private LocalizedProperties getProperties(TreeLogger logger, String prefix,
      GwtLocale locale, GeneratorContext genCtx) {
    ...
    try {
      str = Resource.toStreamOrNull(genCtx.tryFindResourceUrl(logger, propFile));
      if (str != null) {
