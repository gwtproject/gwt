...
/**
 * List formatting for the "eu" locale.
 */
public class ListPatternsImpl_eu implements ListPatterns {
  
  @Override
  public String formatEntry(int index, int count, String left, String formattedTail) {
    switch (count) {
      case 2:
        return left + " eta " + formattedTail;
    }
    if (index == 0) {
  ...
  public SafeHtml formatEntry(int index, int count, SafeHtml left, SafeHtml formattedTail) {
    switch (count) {
      case 2:
        return new com.google.gwt.safehtml.shared.SafeHtmlBuilder().append(left).appendHtmlConstant(" eta ").append(formattedTail).toSafeHtml();
    }
    if (index == 0) {
