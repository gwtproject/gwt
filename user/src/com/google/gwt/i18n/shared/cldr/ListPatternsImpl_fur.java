...
/**
 * List formatting for the "fur" locale.
 */
public class ListPatternsImpl_fur implements ListPatterns {
  
  @Override
  public String formatEntry(int index, int count, String left, String formattedTail) {
    switch (count) {
      case 2:
        return left + " e " + formattedTail;
    }
    if (index == 0) {
  ...
  public SafeHtml formatEntry(int index, int count, SafeHtml left, SafeHtml formattedTail) {
    switch (count) {
      case 2:
        return new com.google.gwt.safehtml.shared.SafeHtmlBuilder().append(left).appendHtmlConstant(" e ").append(formattedTail).toSafeHtml();
    }
    if (index == 0) {
