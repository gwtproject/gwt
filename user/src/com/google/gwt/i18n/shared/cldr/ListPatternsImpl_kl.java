...
/**
 * List formatting for the "kl" locale.
 */
public class ListPatternsImpl_kl implements ListPatterns {
  
  @Override
  public String formatEntry(int index, int count, String left, String formattedTail) {
    switch (count) {
      case 2:
        return left + " aamma " + formattedTail;
    }
    if (index == 0) {
      return left + " aamma " + formattedTail;
    }
  ...
  public SafeHtml formatEntry(int index, int count, SafeHtml left, SafeHtml formattedTail) {
    switch (count) {
      case 2:
        return new com.google.gwt.safehtml.shared.SafeHtmlBuilder().append(left).appendHtmlConstant(" aamma ").append(formattedTail).toSafeHtml();
    }
    if (index == 0) {
      return new com.google.gwt.safehtml.shared.SafeHtmlBuilder().append(left).appendHtmlConstant(" aamma ").append(formattedTail).toSafeHtml();
    }
