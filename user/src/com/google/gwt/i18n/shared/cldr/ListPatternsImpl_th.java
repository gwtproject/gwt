...
/**
 * List formatting for the "th" locale.
 */
public class ListPatternsImpl_th implements ListPatterns {
  
  @Override
  public String formatEntry(int index, int count, String left, String formattedTail) {
    switch (count) {
      case 2:
        return left + "และ" + formattedTail;
    }
    if (index == 0) {
  ...
  public SafeHtml formatEntry(int index, int count, SafeHtml left, SafeHtml formattedTail) {
    switch (count) {
      case 2:
        return new com.google.gwt.safehtml.shared.SafeHtmlBuilder().append(left).appendHtmlConstant("และ").append(formattedTail).toSafeHtml();
    }
    if (index == 0) {
