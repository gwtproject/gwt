...
public class JUnitShell extends DevMode {
  ...
  public static Set<String> getRemoteUserAgents() {
    ...
    if (unitTestShell.userAgentsOpt != null) {
      return unitTestShell.userAgentsOpt;
    }

    String userAgent = unitTestShell.runStyle.getUserAgent();
    if (userAgent != null) {
      return Collections.<String>singleton(userAgent);
    }
    return null;
  }
  ...
  protected boolean doStartup() {
    ...
    int rv = createRunStyle(runStyleName);
    if (rv < 0) {
    ...
    if (rv > 1) {
      getTopLogger().log(TreeLogger.ERROR,
          "Running multiple browsers with a run style is no longer supported");
      return false;
    }
  ...
  protected boolean notDone() {
    ...
    if (!runStyle.isBrowserAlive()) {
      throw new TimeoutException("Remote browser died a mysterious death.");
    }
