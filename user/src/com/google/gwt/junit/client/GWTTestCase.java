...
public abstract class GWTTestCase extends TestCase {
   ...
   * implementation causes test case to be reported as 'failed' however in some rare situations
   * where an uncaught exception is expected then test case may choose to alter the behavior by
   * overriding it:
   * <pre>
   ...
   *     if (t instanceof ExpectedUmbrellaException) {
   *       return;
   *     }
   ...
   * still executing. In that case, test will fail after the main body returns. If the test has
   * already finished before this method is called, the behavior is undefined.
   */
