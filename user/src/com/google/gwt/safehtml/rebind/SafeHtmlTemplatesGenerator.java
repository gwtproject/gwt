...
import static com.google.gwt.user.rebind.AbstractSourceCreator.error;
import static com.google.gwt.user.rebind.AbstractSourceCreator.wrap;
...
public class SafeHtmlTemplatesGenerator extends Generator {
  ...
  public String generate(TreeLogger logger, GeneratorContext genCtx,
    ...
    if (printWriter != null) {
      ...
      for (JMethod method : interfaceType.getOverridableMethods()) {
        writeTemplateMethod(logger, sourceWriter, method);
      }
  ...
  private void writeTemplateMethod(TreeLogger logger, SourceWriter sw, JMethod method)
      throws UnableToCompleteException {
    if (!method.getReturnType().getQualifiedSourceName().equals(SAFE_HTML_FQCN)) {
      throw error(logger, "All methods in interfaces extending "
          + "SafeHtmlTemplates must have a return type of " + SAFE_HTML_FQCN + ".");
    }
    ...
    sw.print("public %s %s(", SAFE_HTML_FQCN, method.getName());
    int i = 0;
    for (JParameter param : method.getParameters()) {
      if (i > 0) {
        sw.print(", ");
      }
      sw.print("%s arg%d", param.getType().getParameterizedQualifiedSourceName(), i++);
    }
    sw.println(") {");
    sw.indent();

    emitMethodBodyFromTemplate(logger, sw, templateAnnotation.value(), method.getParameters());
   ...
   *        parameter
   */
  private void emitAttributeContextParameterExpression(TreeLogger logger, SourceWriter sw,
      HtmlContext htmlContext, String formalParameterName, JType parameterType) {
   ...
   */
  private void emitMethodBodyFromTemplate(TreeLogger logger, SourceWriter sw, String template,
      JParameter[] params) throws UnableToCompleteException {
    sw.println("StringBuilder sb = new java.lang.StringBuilder();");
    ...
    for (TemplateChunk chunk : parser.getParsedTemplate().getChunks()) {
      if (chunk.getKind() == TemplateChunk.Kind.LITERAL) {
        emitStringLiteral(sw, ((LiteralChunk) chunk).getLiteral());
        ...
        emitParameterExpression(
            logger, sw, parameterChunk.getContext(), formalParameterName, paramType);
      } else {
    ...
    sw.outdent();
    sw.outdent();
    sw.println("return new " + BLESSED_STRING_FQCN + "(sb.toString());");
  }
   ...
   *           htmlContext
   */
  private void emitParameterExpression(TreeLogger logger, SourceWriter sw, HtmlContext htmlContext,
    ...
    switch (contextType) {
      case CSS:
        ...
        emitTextContextParameterExpression(sw, formalParameterName, parameterType);
        break;
      case TEXT:
        emitTextContextParameterExpression(sw, formalParameterName, parameterType);
        break;
      ...
      case CSS_ATTRIBUTE_START:
        ...
        emitAttributeContextParameterExpression(logger, sw, htmlContext, formalParameterName,
            parameterType);
        break;
   ...
   */
  private void emitStringLiteral(SourceWriter sw, String str) {
    sw.print("sb.append(");
    sw.print(wrap(str));
    sw.println(");");
  }
   ...
   *        parameter
   */
  private void emitTextContextParameterExpression(SourceWriter sw, String formalParameterName,
      JType parameterType) {
    boolean parameterIsPrimitiveType = (parameterType.isPrimitive() != null);
    boolean parameterIsNotStringTyped = !isString(parameterType);
    ...
    if (isSafeHtml(parameterType)) {
      ...
      sw.print(formalParameterName + ".asString()");
      ...
      sw.print(SAFE_HTML_UTILS_FQCN + ".htmlEscape(" + expression + ")");
    }
  }
  ...
  /**
   * Check if the specified parameter type represents a {@link String}.
   *
   * @param parameterType the Java parameter type
   * @return true if the type represents a {@link String}
   */
  private boolean isString(JType parameterType) {
    return parameterType.getQualifiedSourceName().equals(JAVA_LANG_STRING_FQCN);
  }
}
