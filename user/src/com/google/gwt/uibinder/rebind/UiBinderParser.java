...
public class UiBinderParser {
  ...
  private void createResource(XMLElement elem) throws UnableToCompleteException {
    ...
    if (resourceType != null && writer.getOwnerClass().getUiFactoryMethod(resourceType) != null && !hasAttributes) {
  ...
  private void createResourceUiFactory(XMLElement elem, String resourceName, JClassType resourceType)
    ...
    if (factoryMethod.getParameters().length != 0) {
      writer.die(elem, "Missing %s factory method parameters. Use ui:attributes tag to specify the required parameters.", resourceType);
    }
