...
public class History {
  ...
  private static native String decodeFragment(String s) /*-{
    return $wnd.decodeURI(s.replace("%23", "#"));
  }-*/;

  private static native JavaScriptObject getEntryWrappedFunctionForOurHandler() /*-{
    return $entry(@com.google.gwt.user.client.History::onHashChanged());
  }-*/;

  private static native String getHash() /*-{
    // not using Window.Location.getHash() in order to reduce dependencies
    // and we do not have setHash anyway
    return $wnd.location.hash;
  }-*/;

  private static void initialize() {
    String sanitizedToken = sanitizeToken(getHash());
    setToken(sanitizedToken);
    impl.attachListener(getEntryWrappedFunctionForOurHandler());
  }
  ...
  private static void onHashChanged() {
    String sanitizedToken = sanitizeToken(getHash());
    if (!sanitizedToken.equals(getToken())) {
      setToken(sanitizedToken);
      historyEventSource.fireValueChangedEvent(sanitizedToken);
    }
    impl.maybeRunOtherListeners();
  }

  private static String sanitizeToken(String hash) {
    if (hash.length() > 0) {
      return decodeFragment(hash.substring(1));
    }
    return "";
  }
