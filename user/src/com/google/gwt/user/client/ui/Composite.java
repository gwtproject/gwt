 ...
 * calls to the wrapped widget (e.g. HasWidgets APIs). As an alternative, subclasses can choose a
...
public abstract class Composite<T extends Widget> extends Widget implements IsRenderable {
  ...
  /**
   * Returns the topmost widget that defines this composite.
   */
  ...
  /**
   * Returns the delegate widget passed to {@code initWidget}. Subclasses may
   * forward API calls to this delegate.
   */
   ...
   * to a panel. {@code initWidget} may only be called once for a given
   * composite.
   * <p>
   * The same widget will be wrapped and used as a delegate. if you want to use
   * a different delegate, call {@link #initWidget(Widget, Widget)} instead.
   *
