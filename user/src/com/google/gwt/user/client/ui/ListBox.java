...
public class ListBox extends FocusWidget implements SourcesChangeEvents,
  ...
  private static final String IS_NOT_OPT_GROUP = "is not an optgroup element";
  ...
  public void insertGroup(String label, String[] items, 
      ...
      boolean disabled) {
    assert items != null : "items can not be null";
    ...
    for (int i = 0; i < items.length; i++) {
      ...
      setOptionText(optionElement, item, dir);
      optionElement.setValue(value);
      optGroupElement.appendChild(optionElement);
    }
    SelectElement selectElement = getElement().cast();
    int length = selectElement.getChildCount();
    index = index < 0 || index > length ? length : index;
    if (index == length) {
   ...
   */
  public boolean isGroupDisabled(int index) {
    assert isGroup(index) : IS_NOT_OPT_GROUP;
  ...
  public boolean isGroup(int index) {
    checkGroupIndex(index);
    String nodeName = getSelectElement().getChild(index).getNodeName();
    return nodeName.equalsIgnoreCase(OPT_GROUP);
  }
   ...
   * <p>
   * <em>Note: This method removes the first occurrence 
   * of groups with the same name.</em>
   * </p>
   * 
   * @param label name of the group to be removed.
   */
  public void removeGroup(String label) {
    removeGroup(label, false);
  }
   ...
   * 
   * @param label name of the group to be removed.
   * @param removeAll <code>true</code> removes all groups with the same name, 
   *        <code>false</code> removes the first occurrence.
   */
  public void removeGroup(String label, boolean removeAll) {
    checkLabel(label);
    NodeList<Element> nodeList = getSelectElement().getElementsByTagName(OPT_GROUP);
    for (int i = nodeList.getLength() - 1; i >= 0; i--) {
      OptGroupElement optGroupElement = (OptGroupElement) nodeList.getItem(i);
      if (label.equalsIgnoreCase(getOptGroupLabel(optGroupElement))) {
        nodeList.getItem(i).removeFromParent();
        if (!removeAll) {
          return;
        }
   ...
   */
  public void setGroupDisabled(int index, boolean disabled) {
    assert isGroup(index) : IS_NOT_OPT_GROUP;
    OptGroupElement optgroup = (OptGroupElement) getSelectElement().getChild(index);
   ...
   */
  public void setGroupLabel(int index, String newLabel) {
    assert isGroup(index) : IS_NOT_OPT_GROUP;
    checkLabel(newLabel);
    OptGroupElement optgroup = ((OptGroupElement) getSelectElement().getChild(index));
   ...
   * <p>
   * <em>Note: This method replaces the first occurrence.</em>
   * </p>
   * 
   ...
   * 
   */
  public void setGroupLabel(String oldLabel, String newLabel) {
    setGroupLabel(oldLabel, newLabel, false);
  }
   ...
   * @param oldLabel old group name
   * @param newLabel new group name
   * @param replaceAll <code>true</code> replace all groups with the same name, <code>false</code>
   *          replaces the first occurrence.
   */
  public void setGroupLabel(String oldLabel, String newLabel, boolean replaceAll) {
    checkLabel(oldLabel);
    checkLabel(newLabel);
    NodeList<Element> nodeList = getSelectElement().getElementsByTagName(OPT_GROUP);
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (label.equalsIgnoreCase(oldLabel)) {
        setOptGroupLabel(optgroup, newLabel, null);
        if (!replaceAll) {
          return;
        }
  ...
  private final void checkGroupIndex(int index) {
    int length = getSelectElement().getChildCount();
  ...
  private final void checkLabel(String label) {
    assert label != null && !label.isEmpty() : "label is mandatory";
  }
