  ...
  private final class MenuPopup extends DecoratedPopupPanel {
    ...
    private void setPositionInClient(int left, int top) {
      ...
      if (getOffsetHeight() < Window.getClientHeight()) {
        top = Math.min(top, getClientBottom() - getOffsetHeight());
        top = Math.max(getClientTop(), top);
      }
    ...
    private int getClientTop() {
      return Window.getScrollTop();
    }

    private int getClientBottom() {
      return getClientTop() + Window.getClientHeight();
    }
  }
