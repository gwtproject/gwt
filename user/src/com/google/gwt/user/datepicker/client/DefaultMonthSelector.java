...
public final class DefaultMonthSelector extends MonthSelector {
  ...
  private void createMonthSelect() {
  ...
  private void setMonth(Date date) {
    boolean isMonthSelectable = getDatePicker().isMonthSelectable();
    boolean isYearSelectable = getDatePicker().isYearSelectable();

    int monthColumn = getModel().isMonthBeforeYear() || (!isMonthSelectable && !isYearSelectable) ? 2 : 3;

    if (!getDatePicker().isYearNavigationEnabled()) {
      monthColumn--;
    }
  ...
  private void setYear(Date date) {
    if (getDatePicker().isYearSelectable()) {
      ...
      int yearColumn = getModel().isMonthBeforeYear() ? 3 : 2;
