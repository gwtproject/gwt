...
class SerializationPolicyClient {
  ...
  SerializationPolicyClient(int connectTimeout, int readTimeout) {
  ...
  private static void logMissingClasses(Logger logger, List<ClassNotFoundException> errs) {
    logger.logInfo("unable to load server-side classes used by policy:");

    int limit = Math.min(10, errs.size());
    for (int i = 0; i < limit; i++) {
      logger.logInfo(errs.get(i).getMessage());
    }
    int omitted = errs.size() - limit;
    if (omitted > 0) {
      logger.logInfo("(omitted " + omitted + " more classes)");
    }
  }
