...
public class ProxyAutoBean<T> extends AbstractAutoBean<T> {
      ...
      if (toReturn == null) {
        ...
        for (Method method : beanType.getMethods()) {
          if (BeanMethod.GET.matches(method)) {
            ...
            Type genericReturnType = resolveGenerics(beanType, method.getGenericReturnType());
  ...
  private static Type resolveGenerics(Class<?> containingType, Type type) {
    ...
    if (type instanceof TypeVariable<?>) {
      ...
      if (genericDeclaration instanceof Class<?>) {
        ...
        Type[] types = TypeUtils.getParameterization(declaration, containingType);
        ...
        // instead?
        return TypeUtils.ensureBaseType(type);
      }
    }
