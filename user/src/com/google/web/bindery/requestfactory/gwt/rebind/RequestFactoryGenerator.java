  ...
  private void writeContextImplementations() {
    for (ContextMethod method : model.getMethods()) {
      ...
      for (RequestMethod request : method.getRequestMethods()) {
        ...
        sw.println("class X extends %s<%s, %s> implements %s {", AbstractRequest.class
            .getCanonicalName(), request.getInstanceType() == null ?
            BaseProxy.class.getCanonicalName() : request.getInstanceType().getQualifiedSourceName(),
