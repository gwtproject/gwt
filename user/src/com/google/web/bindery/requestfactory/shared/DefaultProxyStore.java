...
public class DefaultProxyStore implements ProxyStore {
  ...
  public DefaultProxyStore(String payload) throws IllegalArgumentException {
    ...
    // #encode() saves nextId + 1, so we remove 1 here; see rationale below.
    nextId = message.getSyntheticId() - 1;
    if (nextId < 0) {
      /*
       * In case this is a message generated by a pre-2.5 DefaultProxyStore,
       * getSyntheticId will be '0', so we initialize the nextId to our best
       * guess (that only works because the store is append-only, and it could
       * break if nextId() was called on the original DefaultProxyStore more
       * times than put()).
       * Post-2.5 DefaultProxyStore never stores a '0' (see #encode()).
       */
      nextId = map.size();
    }
  }
  ...
  public String encode() {
    // Backwards-compat: never store a '0' (see constructor)
    assert nextId >= 0;
    messageBean.as().setSyntheticId(nextId + 1);
