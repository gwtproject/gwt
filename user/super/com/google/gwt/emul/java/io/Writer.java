...
/**
 * The base class for all writers. A writer is a means of writing data to a target in a
 * character-wise manner. Most output streams expect the {@link #flush()} method to be called before
 * closing the stream, to ensure all data is actually written out.
 *
 * <p>This abstract class does not provide a fully working implementation, so it needs to be
 * subclassed, and at least the {@link #write(char[], int, int)}, {@link #close()} and {@link
 * #flush()} methods needs to be overridden. Overriding some of the non-abstract methods is also
 * often advised, since it might result in higher efficiency.
 *
 * <p>Many specialized readers for purposes like reading from a file already exist in this package.
 *
 * @see Reader
 */
public abstract class Writer implements Appendable, Closeable, Flushable {
  ...
  /**
   * Constructs a new {@code Writer} with {@code this} as the object used to synchronize critical
   * sections.
   */
  protected Writer() {
    lock = this;
  ...
  /**
   * Closes this writer. Implementations of this method should free any resources associated with
   * the writer.
   *
  ...
  /**
   * Flushes this writer. Implementations of this method should ensure that all buffered characters
  ...
  /**
   * Writes one character to the target. Only the two least significant bytes of the integer {@code
   * oneChar} are written.
   *
  ...
  /**
   * Appends the character {@code c} to the target. This method works the same way as {@link
   * #write(int)}.
   *
   ...
   * {@code Writer.write(csq.toString())}. If {@code csq} is {@code null}, then the string "null" is
   ...
   * the same way as {@code Writer.writer(csq.subsequence(start, end).toString())}. If {@code csq}
   * is {@code null}, then the specified subsequence of the string "null" will be written to the
   * target.
   *
   * @param csq the character sequence appended to the target.
  ...
  /**
   * Returns true if this writer has encountered and suppressed an error. Used by PrintWriters as an
   * alternative to checked exceptions.
   */
  boolean checkError() {
    return false;
  }
