...
public abstract class Number implements Serializable {
   ...
   * and validating the result.
   */
  protected static long __parseAndValidateLong(String s, boolean unsigned, int radix) throws NumberFormatException {
    if (s == null) {
      throw NumberFormatException.forNullInputString();
    }
    if (radix < Character.MIN_RADIX || radix > Character.MAX_RADIX) {
    ...
    int maxSignedLen = __ParseLong.maxLengthForRadix[radix];
    if (!unsigned && length > maxSignedLen) {
      throw NumberFormatException.forInputString(orig);
    }
    if (unsigned && length > __ParseUnsignedLong.maxLengthForRadix[radix]) {
      throw NumberFormatException.forInputString(orig);
    }
    ...
    // extract the first digit of the number to parse it as signed long and
    // then combine them together.
    int unsignedTailDigit = -1;
    if (unsigned && length >= maxSignedLen) {
      unsignedTailDigit = Character.digit(s.charAt(length - 1), radix);
    ...
    if (unsignedTailDigit != -1) {
      ...
      if (toReturn < 0 ||
          maxDividend < toReturn ||
          (maxDividend == toReturn && maxRemainder < unsignedTailDigit)) {
        throw NumberFormatException.forInputString(orig);
      }
      toReturn = toReturn * radix + unsignedTailDigit;
    }
