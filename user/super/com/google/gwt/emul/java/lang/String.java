...
public final class String implements Comparable<String>, CharSequence,
  ...
  private native Object exec(Object regex, String value) /*-{
  ...
  private native int matchObjectGetIndex(Object matchObject) /*-{
    return matchObject.index;
  }-*/;

  private native int matchObjectGetMatchLength(Object matchObject) /*-{
    return matchObject[0].length;
  }-*/;

  private native int regExResetLastIndex(Object compiledRegEx) /*-{
    compiledRegEx.lastIndex = 0;
  }-*/;

  private native String[] createNativeStringArray() /*-{
    return [];
  }-*/;
  ...
  public String[] split(String regex, int maxMatch) {
    ...
    Object compiled = createRegEx(regex, "g");
    // the Javascipt array to hold the matches prior to conversion
    String[] out = createNativeStringArray();
    ...
    while (true) {
      ...
      // subgroup handling
      Object matchObj = exec(compiled, trail);
      if (matchObj == null || trail == "" || (count == (maxMatch - 1) && maxMatch > 0)) {
        out[count] = trail;
        break;
      } else {
        out[count] = trail.substring(0, matchObjectGetIndex(matchObj));
        trail = trail.substring(
            matchObjectGetIndex(matchObj) + matchObjectGetMatchLength(matchObj), trail.length());
        // Force the compiled pattern to reset internal state
        regExResetLastIndex(compiled);
    ...
    // The out array is actually a JavaScript array, this makes sure that it has proper type
    // information attached to it
    ArrayStamper.stampJavaTypeInfo(out, new String[0]);
    return out;
  }
