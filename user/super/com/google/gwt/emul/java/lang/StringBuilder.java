...
public class StringBuilder implements CharSequence, Appendable {
  ...
  /**
   * Visible to allow {@link StringBuffer} to reuse it.
   */
  ...
  public StringBuilder append(char[] x, int start, int len) {
    data += String.valueOf(x, start, len);
    return this;
  }
  ...
  public StringBuilder appendCodePoint(int x) {
    data += String.valueOf(Character.toChars(x));
    return this;
  }
   ...
   * <p>Initializing variables in the class body leads to hidden class changes
   * and need to be avoided in performance critical code.
   */
  private void init() {
    data = "";
  }
