...
abstract class AbstractNavigableMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V> {
  ...
  private static <K, V> SimpleImmutableEntry<K, V> exportEntry(Entry<K, V> entry) {
  ...
  boolean containsEntry(Entry<?, ?> entry) {
    Entry<K, V> lookupEntry = getEntry((K) entry.getKey());
  ...
  private SimpleImmutableEntry<K, V> pollEntry(Entry<K, V> entry) {
    if (entry != null) {
      remove(entry.getKey());
    }
