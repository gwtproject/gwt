...
public class ArrayDeque<E> extends AbstractCollection<E> implements Deque<E>,
   ...
   */
  private transient E[] elements = (E[]) new Object[0];
  ...
  /**
   * Checks calculated capacity overflow
   */
  ...
  private <T> T[] copyElements(T[] out) {
    if (head < tail) {
      System.arraycopy(elements, head, out, 0, size());
    } else {
      int headPortionLen = elements.length - head; // number of elements to the right of head
      System.arraycopy(elements, head, out, 0, headPortionLen);
      System.arraycopy(elements, 0, out, headPortionLen, tail);
    }
    return out;
  }
  ...
  private boolean delete(int i) {
    ...
    int front = (i - head) & length;
    int back  = (tail - i) & length;
    ...
    if (front < back) {
      if (head <= i) {
        ...
        System.arraycopy(elements, head, elements, head + 1, length - head);
      }
      elements[head] = null;
      head = (head + 1) & length;
      return false;
  ...
  private void doubleCapacity() {
    ...
    elements = (E[]) copyElements(new Object[newCapacity]);
    head = 0;
    tail = length;
  }
