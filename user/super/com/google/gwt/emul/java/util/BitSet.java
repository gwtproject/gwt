...
public class BitSet {
  ...
  // clears one bit
  private static void clear(int[] array, int bitIndex) {
  ...
  // clones the int[] array
  private static native int[] clone(int[] array) /*-{
    return array.slice(0);
  }-*/;

  // flips one bit
  private static void flip(int[] array, int bitIndex) {
    ...
    if (((word >>> offset) & 1) == 1) {
      ...
      // if off, turn it on
      array[index] = word | (1 << offset);
    }
  }

  // gets one bit
  private static boolean get(int[] array, int bitIndex) {
  ...
  private static void set(int[] array, int bitIndex) {
    int index = wordIndex(bitIndex);
    array[index] = getWord(array, index) | (1 << (bitOffset(bitIndex)));
  }

  // sets all bits to true within the given range
  private static void set(int[] array, int fromIndex, int toIndex) {
  ...
  private static native int[] slice(int[] array,
  ...
  private static int trimToSize(int[] array) {
    ...
    int last = length - 1;
    if (!isUndefined(array, last)) {
      return last;
    }

    int lastUsedIndex = lastUsedIndex(array);
    if (lastUsedIndex == -1) {
      return lastUsedIndex;
    }

    setArrayLength(array, lastUsedIndex + 1);
    return lastUsedIndex;
  }

  private static native boolean isUndefined(int[] array, int index) /*-{
    return array[index] === undefined;
  }-*/;

  private static native void setArrayLength(int[] array, int length) /*-{
    array.length = length;
  }-*/;

  private static native int lastUsedIndex(int[] array) /*-{
  ...
  private static native void deleteWord(int[] array, int index) /*-{
    delete array[index];
  }-*/;

  // flips all bits stored at a certain index
  private static void flipWord(int[] array, int index) {
    int word = getWord(array, index);
    if (word == 0) {
  ...
  private static void flipMaskedWord(int[] array, int index,
      int from, int to) {
    if (from == to) {
      return;
    }
  ...
  private static native int getWord(int[] array, int index) /*-{
    // OR converts an undefined to 0
    return array[index] | 0;
  }-*/;

  // sets all bits to true at a certain index within the given bit range
  private static void maskInWord(int[] array, int index,
      int from, int to) {
  ...
  private static void maskOutWord(int[] array, int index,
      int from, int to) {
    int word = getWord(array, index);
  ...
  private static int nextSetWord(int[] array, int index) {
    ...
    return findLocalMinimum(array, index);
  }

  private static native int findLocalMinimum(int[] array, int index) /*-{
    var keys = Object.keys(array);
  ...
  private static void setWord(int[] array, int index, int value) {
  ...
  private static native void setLengthWords(int[] array, int length) /*-{
    array.length = length;
  }-*/;
  ...
  public void and(BitSet set) {
    ...
    while ((index = nextSetWord(array, index)) != -1) {
      setWord(array, index,
          array[index] & getWord(set.array, index));
      index++;
    }
  }
  ...
  public void andNot(BitSet set) {
    ...
    while ((index = nextSetWord(set.array, index)) != -1) {
      setWord(array, index,
          getWord(array, index) & ~set.array[index]);
      if (++index >= length) {
  ...
  public boolean intersects(BitSet set) {
    ...
    int index = 0;
    while ((index = nextSetWord(array, index)) != -1) {
      if ((array[index] & getWord(set.array, index)) != 0) {
        return true;
      }
      if (++index >= length) {
