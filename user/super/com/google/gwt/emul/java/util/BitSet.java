...
public class BitSet {
  ...
  private BitSet(int[] array) {
    this.array = array;
    saveLastSetWordIndex(array.length - 1);
  }
  ...
  public void and(BitSet set) {
    ...
    int lastIndex = lastSetWordIndex;
    int minLastIndex = Math.min(lastIndex, set.lastSetWordIndex);
    int index = 0;
    for (; index <= minLastIndex; index++) {
      int word = wordAt(array, index);
      if (word != 0) {
        array[index] = word & wordAt(set.array, index);
      }
    }
    Arrays.fill(array, index, lastIndex + 1, 0);
    saveLastSetWordIndex(index - 1);
  }
  ...
  public void andNot(BitSet set) {
    ...
    int lastIndex = lastSetWordIndex;
    int minLastIndex = Math.min(lastIndex, set.lastSetWordIndex);
    for (int index = 0; index <= minLastIndex; index++) {
    ...
    if (minLastIndex != -1) {
      saveLastSetWordIndex(minLastIndex);
    }
  }
  ...
  public void clear() {
    if (lastSetWordIndex != -1) {
      Arrays.fill(array, 0, lastSetWordIndex + 1, 0);
      lastSetWordIndex = -1;
    }
  }
  ...
  public void clear(int fromIndex, int toIndex) {
    ...
    int first = wordIndex(fromIndex);
    if (first > lastSetWordIndex) {
      return;
    }

    int last = wordIndex(toIndex);
    if (last > lastSetWordIndex) {
      last = lastSetWordIndex;
      toIndex = length();
    }
    ...
    if (last == lastSetWordIndex) {
      if (first != last && wordAt(array, last) == 0) {
        saveLastSetWordIndex(first);
      } else {
        saveLastSetWordIndex(last);
      }
    }
  }

  public Object clone() {
    return new BitSet(Arrays.copyOf(array, lastSetWordIndex + 1));
  }
  ...
  public boolean equals(Object obj) {
    ...
    int lastIndex = lastSetWordIndex;
    if (lastIndex != other.lastSetWordIndex) {
      return false;
    }
  ...
  public void flip(int fromIndex, int toIndex) {
    ...
    if (fromIndex >= length) {
      ...
      lastSetWordIndex = wordIndex(toIndex);
      return;
    }
  ...
  public boolean get(int bitIndex) {
    ...
    return index <= lastSetWordIndex && ((wordAt(array, index) >>> bitOffset(bitIndex)) & 1) == 1;
  }
  ...
  public int hashCode() {
    ...
    final int lastIndex = lastSetWordIndex;
  ...
  public void or(BitSet set) {
    ...
    int otherLastIndex = set.lastSetWordIndex;
    maybeGrowArrayToIndex(array, otherLastIndex);
    ...
    // over set b
    int lastIndex = lastSetWordIndex;
    int minLastIndex = Math.min(lastIndex, otherLastIndex);
    int index = 0;
    for (; index <= minLastIndex; index++) {
    ...
    for (; index <= otherLastIndex; ++index) {
      array[index] = wordAt(set.array, index);
    }
    saveLastSetWordIndex(Math.max(lastIndex, otherLastIndex));
  }
  ...
  public void set(int fromIndex, int toIndex) {
    ...
    if (fromIndex != toIndex) {
      setInternal(array, fromIndex, toIndex);
      lastSetWordIndex = Math.max(lastSetWordIndex, wordIndex(toIndex));
    }
  }
  ...
  /**
   * Saves the index of the last word containing a true bit in an array.
   *
   */
  private void saveLastSetWordIndex(int possibleLastSetWordIndex) {
    int i = possibleLastSetWordIndex;
