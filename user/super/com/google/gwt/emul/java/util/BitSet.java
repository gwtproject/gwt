...
public class BitSet {
   ...
   * right to left and starts with one. This helps leaving the rightmost bit as zero to ensure that
   * each word is a tagged small integer in V8.
   *
   * @param bitIndex The bit index.
   * @return The offset of the bit within a word, counting from right to left and starting
   *     with one.
   */
  private static int bitOffset(int bitIndex) {
    return bitIndex % 31 + 1;
  }
  ...
  private void setInternal(int fromIndex, int toIndex) {
    ...
    if (first == last) {
      ...
      maskInWord(array, last, 1, endBit);
  ...
  public void andNot(BitSet set) {
    ...
    while ((index = nextSetWord(set.array, index)) != -1) {
      array[index] = array[index] & ~set.array[index] & 0xfffffffe;
      if (++index > last) {
  ...
  public void clear(int bitIndex) {
    ...
    int word = array[index];
    if (word != 0) {
      array[index] = word & ~(1 << bitOffset(bitIndex)) & 0xfffffffe;
    }
  }
  ...
  public void clear(int fromIndex, int toIndex) {
    ...
    if (first == last) {
      ...
      maskOutWord(array, last, 1, endBit);
  ...
  public void flip(int bitIndex) {
    ...
    int word = array[index];
    if (((word >>> offset) & 1) == 1) {
      array[index] = word & ~(1 << offset) & 0xfffffffe;
    } else {
      array[index] = (word | (1 << offset)) & 0xfffffffe;
    }
  }
  ...
  public void flip(int fromIndex, int toIndex) {
    ...
    if (first == last) {
      ...
      flipMaskedWord(array, last, 1, endBit);

      // Flip everything in between.
      for (int i = first; i < last; i++) {
        array[i] = ~array[i] & 0xfffffffe;
      }
    }
  }
  ...
  public BitSet get(int fromIndex, int toIndex) {
    ...
    int rightShift = bitOffset(fromIndex) - 1;
  ...
  public int nextClearBit(int fromIndex) {
    ...
    int word = array[index];
    for (int i = fromBit + 1; i < 32; i++) {
      if ((word & (1 << i)) == 0) {
        return bitIndex(index) + i - 1;
      }
    }
    ...
    do {
      index++;
      word = array[index];
    } while (word == 0xfffffffe);
    return bitIndex(index) + Integer.numberOfTrailingZeros(~(word >>>  1));
  }
  ...
  public int nextSetBit(int fromIndex) {
    ...
    return bitIndex(index) + Integer.numberOfTrailingZeros(array[index]) - 1;
  }
  ...
  public void set(int bitIndex) {
    ...
    int index = wordIndex(bitIndex);
    maybeGrowArrayTo(index);
    array[index] = (array[index] | (1 << bitOffset(bitIndex))) & 0xfffffffe;
  }
