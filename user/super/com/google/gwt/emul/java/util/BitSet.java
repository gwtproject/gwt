...
public class BitSet {
  ...
  public void andNot(BitSet set) {
    ...
    // iterate over set b.
    int limit = Math.min(set.array.length, last + 1);
  ...
  public int length() {
    ...
    return bitIndex(last) + (Integer.SIZE - Integer.numberOfLeadingZeros(word));
  }
  ...
  public int nextClearBit(int fromIndex) {
    ...
    int word = wordAt(array, index);
    for (int i = fromBit; i < BITS_PER_WORD; i++) {
      if ((word & (1 << i)) == 0) {
  ...
  public int nextSetBit(int fromIndex) {
    ...
    int word = wordAt(array, index);
    if (word != 0) {
      for (int i = bitOffset(fromIndex); i < BITS_PER_WORD; i++) {
        if ((word & (1 << i)) != 0) {
          return bitIndex(index) + i;
        }
      }
    ...
    return bitIndex(index) + Integer.numberOfTrailingZeros(wordAt(array, index));
  }
