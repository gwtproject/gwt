...
public class Collections {
  ...
  private static <T> void rotateUsingReverse(List<T> list, int distance) {
    ...
    // when firstDistanceElements is 0, the operation Collections.reverse(list) will be applied twice.
    // Given L a collection, L = reverse(reverse(L)). We can skip doing the reverse for that situation.
    if (firstDistanceElements == 0) {
      return;
    }
  ...
  private static <T> void rotateWithoutReverse(List<T> list, int distance) {
    ...
    // distance is 0 means do not move anything.
    if (distance == 0) {
      return;
    }
    boolean allElementsMoved = false;
    int elementsMoved = 0;
