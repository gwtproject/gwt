...
public class Date implements Cloneable, Comparable<Date>, Serializable {
  ...
  private void fixDaylightSavings(int requestedHours) {
    ...
    if ((hours % 24) != (requestedHours % 24)) {
      ...
      // minutes:secs:millis or because we are in the situation (1) and has to be fixed.
      // TODO(rluble): Taking modulos does not seen necessary.
    ...
    // Check for situation (2) by advancing the clock by 1 hour and see if getHours() returns the
    // same.
    // Add an hour.
    double timeInMilliseconds = jsdate.getTime();
    jsdate.setTime(timeInMilliseconds + ONE_HOUR_IN_MILLISECONDS);
    if (hours != jsdate.getHours()) {
      // If the requested time dones falls in the "duplicated" hour due to going back out of
      // daylight savings time, return an object representing the latter time.
      jsdate.setTime(timeInMilliseconds);
    }
  }
}
