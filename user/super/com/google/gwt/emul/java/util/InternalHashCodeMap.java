  ...
  public Iterator<Entry<K, V>> iterator() {
    return new Iterator<Map.Entry<K,V>>() {
      ...
      public void remove() {
        ...
        // - We have two or more items left in the chain -> we need to decrease item index
        //   after removal
        // - We have only one item in the chain -> chain will be dropped,
        //   do not change the itemIndex
        int lengthBeforeRemoval = chain.length;
        InternalHashCodeMap.this.remove(lastEntry.getKey());

        // If we are still in the same chain, our itemIndex just jumped an item. We can fix that
