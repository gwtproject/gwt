  ...
  static class InternalJsHashCodeMapModern<K, V> extends InternalJsHashCodeMap<K, V> {
    ...
    public Iterator<Entry<K, V>> entries() {
      ...
      return new Iterator<Map.Entry<K,V>>() {
        int chainIndex = -1, itemIndex = 0, lastItemIndex = -1;
        ...
        public void remove() {
          ...
          K key = lastChain[lastItemIndex].getKey();
          InternalJsHashCodeMapModern.this.remove(key);
    ...
    private native int[] keys() /*-{
      return Object.keys(this.@InternalJsHashCodeMap::backingMap);
    }-*/;

    private native Entry<K, V>[] get(int i) /*-{
      return this.@InternalJsHashCodeMap::backingMap[i];
    }-*/;
  }
