...
class InternalJsStringMap<K, V> implements Iterable<Entry<K, V>> {
  ...
  public InternalJsStringMap(ES6MapFactory factory, AbstractHashMap<K, V> host) {
    this.backingMap = factory.newJsMap();
    this.host = host;
  }
  ...
  private Entry<K, V> newMapEntry(final ES6IteratorEntry<V> entry, final int lastValueMod) {
