...
public final class Spliterators {
  ...
  static class IteratorSpliterator<T> extends AbstractSpliterator<T> {
    private Collection<? extends T> collection;
    private Iterator<? extends T> it;

    IteratorSpliterator(Collection<? extends T> collection, int characteristics) {
      super(collection.size(), sizeKnownSpliteratorCharacteristics(characteristics));
      this.collection = checkNotNull(collection);
    }

    IteratorSpliterator(Iterator<? extends T> it, long size, int characteristics) {
      super(size, sizeKnownSpliteratorCharacteristics(characteristics));
  ...
  private static final class ArraySpliterator<T> implements Spliterator<T> {
    ...
    ArraySpliterator(Object[] array, int from, int limit, int characteristics) {
      checkCriticalPositionIndexes(from, limit, array.length);
      this.array = array;
      this.index = from;
      this.limit = limit;
  ...
  private static final class DoubleArraySpliterator implements Spliterator.OfDouble {
  ...
  private static void checkSorted(Spliterator<?> spliterator) {
    checkCritcalState(spliterator.hasCharacteristics(Spliterator.SORTED));
  }
