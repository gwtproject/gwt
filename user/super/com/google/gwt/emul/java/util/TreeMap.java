...
public class TreeMap<K, V> extends AbstractNavigableMap<K, V> implements
  ...
  private class SubMap extends AbstractNavigableMap<K, V> {
    ...
    public Node<K, V> firstEntry() {
      ...
      if (type.toKeyValid() && larger(node.getKey(), toKey, toInclusive)) {
        return null;
      }
      return node;
    }
    ...
    public Node<K, V> lastEntry() {
      ...
      if (type.fromKeyValid() && smaller(node.getKey(), fromKey, fromInclusive)) {
        return null;
      }
      return node;
    }
  ...
  private void inOrderAdd(List<Entry<K, V>> list, SubMapType type, Node<K, V> current,
    ...
    if (rightNode != null) {
      inOrderAdd(list, type, rightNode,
          fromKey, fromInclusive, toKey, toInclusive);
    }
  }
  ...
  /**
   * Returns true if <code>a</code> is less than <code>b</code> or equal to, if <code>b</code>
   * is not <code>inclusive</code>.
   */
  private boolean smaller(K a, K b, boolean inclusive) {
    int compare = cmp.compare(a, b);
    return compare < 0 || (!inclusive && compare == 0);
  }
  ...
  /**
   * Returns true if <code>a</code> is greater than <code>b</code> or equal to, if <code>b</code>
   * is not <code>inclusive</code>.
   */
  private boolean larger(K a, K b, boolean inclusive) {
    int compare = cmp.compare(a, b);
    return compare > 0 || (!inclusive && compare == 0);
  }
}
