...
public class UUID implements Serializable, Comparable<UUID> {
  ...
  /**
   * Not implementing on client side due to MD5 dependency
   * 
   * @param name
   * @return
   */
   ...
   * compliant, although we can't really guarantee much about it's 
   * randomness as we do not have a suitable secure random generator
   * in the browser.
   *
   * @return UUID that was randomly generated
   */
  public static UUID randomUUID() {
    long msb = 0, lsb = 0;

    for (int i = 0; i < 16; i++) {
      // Set version byte (4 - Random)
      if (i == 12) {
        msb = (msb << 4) | 0x4;
      } else {
        msb = (msb << 4) | ((int) (Math.random() * 16));
      }
    }

    // Set variant (2 - Leach-Salz)
    lsb = ((int) (Math.random() * 16)) & 0x03 | 0x08;
    for (int i = 1; i < 16; i++) {
      lsb = (lsb << 4) | ((int) (Math.random() * 16));
  ...
  /**
   * In v1 type UUIDs returns the 60-bit timestamp of when the UUID
   * was generated.  This is represented by Coordinated Universal 
  ...
  /**
   * In v1 type UUIDs returns the generated clock sequence. The clock sequence 
   * is used to help avoid duplicates that could arise when the clock is set 
