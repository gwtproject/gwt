...
public final class Collectors {
  ...
  private static Set<Collector.Characteristics> removeIdentFinisher(Set<Collector.Characteristics> characteristics) {
    HashSet<Collector.Characteristics> result = new HashSet<>(characteristics);
...
//  public static <T,K,A,D,M extends ConcurrentMap<K,D>> Collector<T,?,M> groupingByConcurrent(Function<? super T,? extends K> classifier, Supplier<M> mapFactory, Collector<? super T,A,D> downstream)
  ...
  public static Collector<CharSequence,?,String> joining(final CharSequence delimiter, CharSequence prefix, CharSequence suffix) {
    return Collector.<CharSequence, StringJoiner, String>of(
    ...
    return Collector.of(
        ...
        (m1, m2) -> {
          for (Map.Entry<K, U> entry : m2.entrySet()) {
            // can be replaced by Map.merge once that is emulated
            if (m1.containsKey(entry.getKey())) {
              m1.put(entry.getKey(), mergeFunction.apply(m1.get(entry.getKey()), entry.getValue()));
            } else {
              m1.put(entry.getKey(), entry.getValue());
            }
          }
