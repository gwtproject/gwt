...
public abstract class GWTTestCase extends TestCase {
  ...
  protected final void setUp() throws Exception {
    configureUncaughtHandler();
    gwtSetUp();
    assertUncaughtExceptionHandler();
  }

  private void configureUncaughtHandler() {
    if (shouldCatchExceptions()) {
  ...
  protected final void tearDown() throws Exception {
    // TODO(goktug): Fix so that exceptions thrown from tearDown will not be swallowed.
    assertUncaughtExceptionHandler();
    gwtTearDown();
    if (catchUncaughtExceptions()) {
      GWT.setUncaughtExceptionHandler(uncaughtHandler = null);
    }
  }

  private void assertUncaughtExceptionHandler() {
    if (catchUncaughtExceptions() && GWT.getUncaughtExceptionHandler() != uncaughtHandler) {
      throw new IllegalStateException("UncaughtExceptionHandler added by GWTTestCase is replaced. "
          + "GWTTestCase will not be able catch errors reported to UncaughtExceptionHandler.\n"
          + "(see GWTTestCase#catchUncaughtExceptions)");
    }
  }
