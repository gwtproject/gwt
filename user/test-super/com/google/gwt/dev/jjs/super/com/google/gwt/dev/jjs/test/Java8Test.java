...
public class Java8Test extends GWTTestCase {
  ...
  MyFunction2<String, String, String> concat = (t, u) -> (t + u);
  ...
  private static <T> void testMethodReferencesWithGenericTypeParameters(
      Some<T> some, T t1, T t2, MyFunction2<T, T> combine) {
    T t1t2 = combine(t1, t2);
    ...
    assertEquals(t1t2,
        ((MyFunction2<T, MyFunction2<T, T>, Some<T>>) Some<T>::new).apply(t1t2, concat).m1());
  }
}
