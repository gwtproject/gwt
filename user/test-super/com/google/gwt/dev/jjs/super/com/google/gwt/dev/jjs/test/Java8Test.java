...
public class Java8Test extends GWTTestCase {
  ...
  interface StaticFailIfClinitRuns {
    static {
      fail("clinit() shouldn't run from just taking a reference to a method");
    }

    public static Object staticMethod(int x, int y) {
      return null;
    }
  }
  ...
  public void testLambdaNoCapture() {
    assertEquals(130, new AcceptsLambda<Integer>().accept((a, b) -> a + b).intValue() + this.staticField);
  }
  ...
  public void testStaticReferenceBinding() throws Exception {
    assertEquals(30, new AcceptsLambda<Integer>().accept(Statnew AcceptsLambda<Integer>().accept(Static::staticMethod).intValue()ic::staticMethod).intValue());
    // if this next line runs a clinit, it fails
    Lambda l = StaticFailIfClinitRuns::staticMethod;
    try {
