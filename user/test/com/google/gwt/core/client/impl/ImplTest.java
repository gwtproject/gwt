...
public class ImplTest extends GWTTestCase {
  ...
  public void testPinnedByImpl_getNameOf() {
    ...
    if (areNamesObfuscatedOrNotPresent()) {
      // In obfuscated mode the names of the functions in the output code have not relation
    ...
    String prunnableFnName =
        Impl.getNameOf("@com.google.gwt.core.client.impl.ImplTest::prunnableFunction()");
    assertTrue("Expecting 'prunnableFunction' got '" + prunnableFnName + "'",
        prunnableFnName.contains("prunnableFunction"));
  ...
  private static native boolean areNamesObfuscatedOrNotPresent() /*-{
    // Make sure this function does not get inlined by our JavaScriptInliner.
    try {
    } finally {
    }
