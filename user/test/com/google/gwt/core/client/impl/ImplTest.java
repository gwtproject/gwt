...
public class ImplTest extends GWTTestCase {
  ...
  public void testPinnedByImpl_getNameOf() {
    Foo foo = new Foo();
    assertNotNull(foo.statifiableFunction("a" + "b"));
    ...
    if (areNamesPreserved()) {
      String prunnableString =
          Impl.getNameOf("@com.google.gwt.core.client.impl.ImplTest::prunnableFunction()");
      assertTrue("Expecting 'prunnableFunction' got '" + prunnableString + "'",
          prunnableString.contains("prunnableFunction"));

      String inlineableString =
          Impl.getNameOf("@com.google.gwt.core.client.impl.ImplTest::inlinableFunction()");
      assertTrue("Expecting 'inlinableFunction' got '" + inlineableString + "'",
          inlineableString.contains("inlinableFunction"));


      String statisfiableString =
          Impl.getNameOf("@com.google.gwt.core.client.impl.ImplTest.Foo::statifiableFunction(*)");
      assertTrue("Expecting 'statifiableFunction' got '" + statisfiableString + "'",
          statisfiableString.contains("statifiableFunction"));
    }
  }

  private static native boolean areNamesPreserved() /*-{
    try {
    } finally {
    }
    var fn = @ImplTest::areNamesPreserved();
    return fn.name && fn.name.indexOf && fn.name.indexOf("areNamesPreserved") != -1;
  }-*/;
