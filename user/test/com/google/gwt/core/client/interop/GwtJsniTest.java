...
public class GwtJsniTest extends GWTTestCase {
  ...
  interface JsFunction extends JsObject {
    ...
    JsObject call(JsObject thisRef /* make varargs work , Object... args */);
  }
  ...
  interface ArrayLike<T> {
    ...
    @JsProperty(indexed = true)
    void _set(String key, JsObject value);

    @JsProperty(indexed = true)
    void _set(int key, JsObject valueJ);

    @JsProperty(indexed = true)
    int _intGet(String key);
    ...
    @JsProperty(indexed = true)
    JsObject _get(String key);

    @JsProperty(indexed = true)
    JsObject _get(int key);
  }

  @JsType
  interface JsArray<T> extends JsObject, ArrayLike<T> {
  ...
  public void testInterop() {
    ...
    array = GWT.jsni("({foo: 1, bar: 2})");
    assertEquals(2, array._intGet("bar"));

    JsFunction func = GWT.jsni("function() { return 42; }");
    assertTrue(42 == ((JsNumber) func.call(null)).valueOf());
  }
  ...
  public void testDOM() {
    ...
    assertEquals("hello", query.item(1).innerText().trim());
  }
}
