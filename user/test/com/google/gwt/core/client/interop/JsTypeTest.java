...
public class JsTypeTest extends GWTTestCase {
  ...
  protected void gwtSetUp() throws Exception {
    ...
    patchPrototype(MyNativeJsTypeSubclass.class);
  }

  /**
   * Workaround for the fact that the script is injected after defineClass() has been called.
   */
  private native void patchPrototype(Class<MyNativeJsTypeSubclass> myClass) /*-{
    @com.google.gwt.lang.JavaClassHierarchySetupUtil::assignPrototype(*)(
        @java.lang.Class::getPrototypeForClass(Ljava/lang/Class;)(myClass),
        $wnd.JsTypeTest_MyNativeJsType.prototype);
  }-*/;
  ...
  public static native MyNativeJsType createMyNativeJsType() /*-{
    return new $wnd.JsTypeTest_MyNativeJsType();
  }-*/;

  public void testNativeJsTypeSublass() {
    ...
    // TODO(ruble): replace with the following when NativeJsTypes are done
    //   MyNativeJsType myNativeJsType = new MyNativeJsType();
    MyNativeJsType myNativeJsType = createMyNativeJsType();
    // 30 + undefined = NaN
    assertTrue(Double.isNaN(myNativeJsType.sum(30)));
    setProperty(myNativeJsType, "y", 11);
    assertEquals(41, myNativeJsType.sum(30));
    assertTrue(myNativeJsType instanceof MyNativeJsType);
    assertFalse(myNativeJsType instanceof MyNativeJsTypeSubclass);
  }
