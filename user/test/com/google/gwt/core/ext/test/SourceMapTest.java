...
public class SourceMapTest extends TestCase {
  ...
  /**
   * Tests the correspondence between old symbol maps and source maps. Most of the cases would work.
   * But, there are few that cannot be tested:
   * Field accesses in general
   * ClassLiteralHolder fields
   * Static version of some methods
   *
   */
  ...
  private void checkSplitPloints(File origSplitPoints, File splitPointsFile)
    ...
    JSONArray fragments = (JSONArray) jsPoints.get("fragments");
    final Map<Integer, JSONObject> spoints = Maps.newHashMap();
    for (int i = 0; i < fragments.length(); i++) {
      JSONObject spoint = fragments.getJSONObject(i);
      spoints.put(spoint.getInt("id"), spoint);
    }
      ...
      public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        try {
          if (localName.equals("splipoint")) {
            JSONObject splitPoint = spoints.get(Integer.parseInt(attributes.getValue("id")));
            assertEquals(attributes.getValue("location"), splitPoint.getString("splitPoint"));
