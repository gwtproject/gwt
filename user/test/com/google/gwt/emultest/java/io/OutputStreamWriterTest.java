...
public class OutputStreamWriterTest extends GWTTestCase {
  ...
  /**
   * Array of characters that contains unicode characters. Using unicode as using actual unicode
   * character like 'à' fails checkstyle.
   */
  private static final char[] UNICODE_CHAR_ARRAY = {
    '\u0905', '\u0906', '\u0907', '\u0908', '\u1F00', '\u1F01', '\u0251'
  };
  ...
  public void testWriteUnicodeChar() throws IOException {
    writer.write(UNICODE_CHAR_ARRAY, 0, UNICODE_CHAR_ARRAY.length);
    final byte[] expected = (new String(UNICODE_CHAR_ARRAY).getBytes(encodingUTF8Charset));
  ...
  public void testWriteASCIIChar() throws IOException {
    ...
    final byte[] expected = (new String(ASCII_CHAR_ARRAY).getBytes(encodingUTF8Charset));
    assertEquals(expected, baos.toByteArray());
  }
