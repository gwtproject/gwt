...
public abstract class TreeMapTest<K extends Comparable<K>, V> extends TestMap {
  ...
  private static <K, V> void _assertEquals(Map<K, V> expected, Map<K, V> actual) {
    ...
    // TODO: Needs rewrite. TreeMap caches returned values collection so this assertion
    // makes no sense now.
    Collection<V> expectedValues = expected.values();
    Collection<V> actualValues = actual.values();
    if (expectedValues != actualValues) {
      assertFalse(expectedValues.equals(actualValues));
    }
  ...
  public void testEntrySet_entries1_view_modify() {
    ...
    map.put(keys[0], values[0]); // put a value
  ...
  public void testGet_throwsNullPointerException() {
    ...
    try {
      map.get(null);
      // first get of a null key is not required to NPE since no comparisons are needed.
      // TODO: fix it for java >= 7.
      if (!GWT.isClient() && TestUtils.getJdkVersion() > 6) {
  ...
  public void testIterator_next() {
    ...
    assertEquals(keys[1] + "=" + values[1], it.next().toString());
  ...
  public void testPut_throwsNullPointerException() {
    ...
    if (isPutSupported) {
      ...
      try {
        ...
        // first put of a null key is not required to NPE since no comparisons are needed.
        // TODO: fix it for java >= 7.
        if (!GWT.isClient() && TestUtils.getJdkVersion() > 6) {
          assertTrue(useNullKey());
        }
  ...
  public void testRemove_throwsNullPointerException() {
    ...
    if (isRemoveSupported) {
      ...
      try {
        map.remove(null);
        // first remove of a null key is not required to NPE since no comparisons are needed.
        // TODO: fix it for java >= 7.
        if (!GWT.isClient() && TestUtils.getJdkVersion() > 6) {
          assertTrue(useNullKey());
        }
  ...
  public void testRemove_from_iterator() {
    K[] keys = getKeys();
    V[] values = getValues();
    Map<K, V> map = createMap();
    map.put(keys[0], values[0]);
  ...
  public void testValues2() {
    K[] keys = getKeys();
    V[] values = getValues();
    NavigableMap<K, V> treeMap = createNavigableMap();

    treeMap.put(keys[0], values[0]);
    treeMap.put(keys[1], values[1]);
    treeMap.put(keys[2], values[2]);

    K firstKey = treeMap.firstKey();
    SortedMap<K, V> subMap = treeMap.subMap(firstKey, firstKey);
