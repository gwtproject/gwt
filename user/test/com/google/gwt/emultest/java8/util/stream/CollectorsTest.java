...
/**
 * Tests {@link Collectors}
 */
public class CollectorsTest extends EmulTestBase {

  public void testAveragingDouble() {
    Collector<Double, ?, Double> c = Collectors.averagingDouble(Double::doubleValue);
  ...
  public void testAveragingInt() {
    Collector<Integer, ?, Double> c = Collectors.averagingInt(Integer::intValue);
  ...
  public void testAveragingLong() {
    Collector<Long, ?, Double> c = Collectors.averagingLong(Long::longValue);
  ...
  public void testCollectingAndThen() {
    Collector<Object, ?, List<Object>> listIdentityCollector = Collectors.collectingAndThen(Collectors.toList(), Function.identity());
    // same test as toList():
    ...
    Collector<Object, ?, Integer> uglyCount = Collectors.collectingAndThen(Collectors.toList(), List::size);
  ...
  public void testGroupingBy() {
    Collector<String, ?, Map<String, List<String>>> c1 = Collectors.groupingBy(Function.identity());
    ...
    Collector<String, ?, LinkedHashMap<String, Set<String>>> c2 = Collectors.groupingBy(Function.identity(), LinkedHashMap::new, Collectors.toSet());
  ...
  public void testMapping() {
    Collector<String, ?, List<String>> identityMapping = Collectors.mapping(Function.identity(), Collectors.toList());
    // same test as toList():
    // same items (allow dups)
    applyItems(
        Arrays.asList("a", "a"),
    ...
    Collector<Integer, ?, List<String>> numberMapping = Collectors.mapping(s -> "#" + s, Collectors.toList());
  ...
  public void testMaxBy() {
    Collector<String, ?, Optional<String>> c = Collectors.maxBy(Comparator.<String>naturalOrder());
  ...
  public void testMinBy() {
    Collector<String, ?, Optional<String>> c = Collectors.minBy(Comparator.<String>naturalOrder());
  ...
  public void testPartitioningBy() {
    Collector<Boolean, ?, Map<Boolean, List<Boolean>>> c1 = Collectors.partitioningBy(Boolean::valueOf);
    ...
    Collector<Boolean, ?, Map<Boolean, Set<Boolean>>> c2 = Collectors.partitioningBy(Boolean::valueOf, Collectors.toSet());
  ...
  // reducing: counting, minBy/maxBy use this

  public void testSummarizingDouble() {
    Collector<Double, ?, DoubleSummaryStatistics> c = Collectors.summarizingDouble(Double::doubleValue);
  ...
  public void testSummarizingInt() {
    Collector<Integer, ?, IntSummaryStatistics> c = Collectors.summarizingInt(Integer::intValue);
  ...
  public void testSummarizingLong() {
    Collector<Long, ?, LongSummaryStatistics> c = Collectors.summarizingLong(Long::longValue);
  ...
  public void testSummingDouble() {
    Collector<Double, ?, Double> c = Collectors.summingDouble(Double::doubleValue);
  ...
  public void testSummingInt() {
    Collector<Integer, ?, Integer> c = Collectors.summingInt(Integer::intValue);
  ...
  public void testSummingLong() {
    Collector<Long, ?, Long> c = Collectors.summingLong(Long::longValue);
  ...
  // toCollection tested by set/list
  ...
  public void testMap() {
    Collector<String, ?, Map<String, String>> c = Collectors.toMap(Function.identity(), Function.identity());
    ...
    c = Collectors.toMap(Function.identity(), Function.identity(), (s, s2) -> {
      seen.add("first: " + s);
      seen.add("second: " + s2);
      return s + "," + s2;
    });
   ...
   */
  private <T, A, R> void applyItems(R expected, Collector<T, A, R> collector, T t1, T t2, BiPredicate<R, R> equals) {
   ...
   * can simply verify the output. Taken from the Collector class's javadoc.
   */
  private <T, A, R> void applyItems(R expected, Collector<T, A, R> collector, T t1, T t2) {
    assertEquals("failed without splitting", expected, applyItemsWithoutSplitting(collector, t1, t2));
    assertEquals("failed with splitting", expected, applyItemsWithSplitting(collector, t1, t2));
  }

  /**
   * helper for applyItems
   */
  private <T, A, R> R applyItemsWithoutSplitting(Collector<T, A, R> collector, T t1, T t2) {
    Supplier<A> supplier = collector.supplier();
    BiConsumer<A, T> accumulator = collector.accumulator();
    BinaryOperator<A> combiner = collector.combiner();// unused in this impl
   ...
   * helper for applyItems
   */
  private <T, A, R> R applyItemsWithSplitting(Collector<T, A, R> collector, T t1, T t2) {
    Supplier<A> supplier = collector.supplier();
    BiConsumer<A, T> accumulator = collector.accumulator();
    BinaryOperator<A> combiner = collector.combiner();// actually used in this impl
    Function<A, R> finisher = collector.finisher();
  ...
  private <T, A, R> void assertZeroItemsCollectedAs(R expected, Collector<T, A, R> collector) {
    assertZeroItemsCollectedAs(expected, collector, Object::equals);
  }
  private <T, A, R> void assertZeroItemsCollectedAs(R expected, Collector<T, A, R> collector, BiPredicate<R, R> equals) {
    Supplier<A> supplier = collector.supplier();
    BiConsumer<A, T> accumulator = collector.accumulator();// unused in this impl
    BinaryOperator<A> combiner = collector.combiner();// shouldn't really bs used, just handy to poke the internals quick
    Function<A, R> finisher = collector.finisher();

    R actual = finisher.apply(supplier.get());
    assertTrue("expected= " + expected + ", actual= " + actual, equals.test(expected, actual));
    // doesn't actually ever happen, just internal checks
    actual = finisher.apply(combiner.apply(supplier.get(), supplier.get()));
    assertTrue("expected= " + expected + ", actual= " + actual, equals.test(expected, actual));
  }
  private <T, A, R> void assertSingleItemCollectedAs(R expected, Collector<T, A, R> collector, T item) {
    assertSingleItemCollectedAs(expected, collector, item, Object::equals);
  }
  private <T, A, R> void assertSingleItemCollectedAs(R expected, Collector<T, A, R> collector, T item, BiPredicate<R, R> equals) {
    Supplier<A> supplier = collector.supplier();
    BiConsumer<A, T> accumulator = collector.accumulator();
    BinaryOperator<A> combiner = collector.combiner();// shouldn't really be used, just handy to poke the internals quick
    Function<A, R> finisher = collector.finisher();
    ...
    assertTrue("expected= " + expected + ", actual= " + actual, equals.test(expected, actual));// normal test
