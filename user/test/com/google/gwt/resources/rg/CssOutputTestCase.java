...
public class CssOutputTestCase extends TestCase {
  ...
  public void testOutputCssMapArtifact() throws UnableToCompleteException {
    ...
    verify(mockResourceContext, times(2)).getGeneratorContext();
    verifyNoMoreInteractions(mockResourceContext);
    ...
    verify(method).getEnclosingType();
    verify(method).getName();
    verifyNoMoreInteractions(method);

    verify(bundleType).getQualifiedSourceName();
    verifyNoMoreInteractions(bundleType);
  }
  ...
  public void testOutputCssMapArtifactThrowOnTryCreateResource() throws UnableToCompleteException {
    ...
    verify(mockResourceContext).getGeneratorContext();
    verifyNoMoreInteractions(mockResourceContext);

    verify(mockGeneratorContext).tryCreateResource(testLogger, "cssResource/com.test.Bundle.cssMethod.cssmap");
    verifyZeroInteractions(mockGeneratorContext);
  ...
  public void testOutputCssMapArtifactWithTestData() throws UnableToCompleteException {
    ...
    verify(mockJMethod1).getEnclosingType();
    verify(mockJMethod1).getName();
    verifyNoMoreInteractions(mockJMethod1);

    verify(mockJMethod2).getEnclosingType();
    verify(mockJMethod2).getName();
    verifyNoMoreInteractions(mockJMethod2);

    verify(mockJMethod3).getEnclosingType();
    verify(mockJMethod3).getName();
    verifyNoMoreInteractions(mockJMethod3);
