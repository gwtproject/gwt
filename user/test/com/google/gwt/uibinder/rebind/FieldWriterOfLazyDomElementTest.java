...
public class FieldWriterOfLazyDomElementTest extends TestCase {
  ...
  public void testLazyDomElementNotParameterized() throws Exception {
    ...
    verify(ownerFieldType).isParameterized();
    verifyNoMoreInteractions(ownerFieldType);

    verify(ownerField).getName();
    verify(ownerField).getRawType();
    verifyNoMoreInteractions(ownerField);
  }
  ...
  public void testLazyDomElementIncompatibleParameter() throws Exception {
    ...
    verify(ownerField, times(2)).getName();
    verify(ownerField).getRawType();
    verifyNoMoreInteractions(ownerField);

    verify(parameterClass).getTypeArgs();
    verify(parameterClass, times(2)).getQualifiedSourceName();
    verifyNoMoreInteractions(parameterClass);

    verify(templateFieldType).isAssignableTo(arg);
    verifyNoMoreInteractions(templateFieldType);
  }
  ...
  public void testLazyDomElementCompatibleType() throws Exception {
    ...
    verify(parameterClass).getTypeArgs();
    verify(parameterClass).getQualifiedSourceName();
    verifyNoMoreInteractions(parameterClass);

    verify(templateFieldType).isAssignableTo(arg);
    verifyNoMoreInteractions(templateFieldType);

    verify(arg).getQualifiedSourceName();
