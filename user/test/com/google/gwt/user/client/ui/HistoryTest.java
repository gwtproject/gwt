...
public class HistoryTest extends GWTTestCase {
  ...
  public void testReplaceItem() {
    // running this test in IE8 or IE9 while using the default iframe linker will fail
    // Using Window.Location.replace affects the iframe and might cause the browser to display
    // the code frame instead
    if(isI8orIE9()) {
      return;
    }
      ...
      public void onValueChange(ValueChangeEvent<String> event) {
        ...
        switch (state) {
          ...
          case 1: {
            ...
            state = 2;
            History.replaceHistoryItem(historyToken3, true);
            break;
          }
  ...
  public void testReplaceItemNoEvent() {
    ...
    final String historyToken2 = "token 2";
    final String historyToken3 = "token 3";
    final String historyToken3_encoded = "token%203";
    ...
    History.replaceHistoryItem(historyToken3, false);
    assertEquals(historyToken3, History.getToken());

    timer = new Timer() {
      @Override
      public void run() {
        assertEquals(historyToken3_encoded, getCurrentLocationHash());
        finishTest();
      }
    };
