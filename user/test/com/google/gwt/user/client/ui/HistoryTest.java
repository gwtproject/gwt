...
public class HistoryTest extends GWTTestCase {
  ...
  private static String getCurrentLocationHash() {
    // Older browser tended to decode the hash value of the URL if location.hash was read
    // Just to be safe we are reading the href part and parse out the hash value
  ...
  /**
   * Test to make sure that there is no double unescaping of hash values happening.
   * Firefox decodes the hash fragment when reading it, see
   * {@link com.google.gwt.user.client.impl.WindowImplMozilla}.
   */
  @DoNotRunWith(Platform.HtmlUnitUnknown)
